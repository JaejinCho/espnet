...
dict=data/lang_1char_mono_${lm_lang}/train_units.txt
nlsyms=data/lang_1char_mono_${lm_lang}/non_lang_syms.txt

echo "dictionary: ${dict}"
if [ ${stage} -le 2 ]; then
    ### Task dependent. You have to check non-linguistic symbols used in the corpus.
    echo "stage 2: Dictionary and Json Data Preparation"
    mkdir -p data/lang_1char_mono_${lm_lang}/

    echo "make a non-linguistic symbol list for all languages"
    cut -f 2- data/tr_babel_${lm_lang}/text | grep -o -P '\[.*?\]|\<.*?\>' | sort | uniq > ${nlsyms}
    cat ${nlsyms}

    echo "<unk> 1" > ${dict} # <unk> must be 1, 0 will be used for "blank" in CTC
    text2token.py -s 1 -n 1 -l ${nlsyms} data/tr_babel_${lm_lang}/text | cut -f 2- -d" " | tr " " "\n" \
    | sort | uniq | grep -v -e '^\s*$' | awk '{print $0 " " NR+1}' >> ${dict}
    wc -l ${dict}
fi

# You can skip this and remove --rnnlm option in the recognition (stage 5)
lmexpdir=exp/train_rnnlm_2layer_bs256_mono_${lm_lang}
mkdir -p ${lmexpdir}
if [ ${stage} -le 3 ]; then
