...
hrs=20 # data used for transfer learning (in hours)
...
# kalthick_expdir is added since the directory naming is different from
# origianl espnet. Also, recog_set (only 5 main target languages), expdir, lmexpdir is redfined below
recog_set="dt_babel_assamese et_babel_assamese dt_babel_swahili et_babel_swahili\
 dt_babel_lao et_babel_lao dt_babel_tagalog et_babel_tagalog\
 dt_babel_zulu et_babel_zulu"
expdir=exp/${train_set}_${etype}_e${elayers}_subsample${subsample}_unit${eunits}_proj${eprojs}_d${dlayers}_unit${dunits}_${atype}_aconvc${aconv_chans}_aconvf${aconv_filts}_mtlalpha${mtlalpha}_${opt}_bs${batchsize}_mli${maxlen_in}_mlo${maxlen_out}_karthick_${hrs}hrs_transfer_rnnlm_added
if [ ${stage} -le 5 ]; then
    echo "stage 5: Decoding"
    nj=32
    for rtask in ${recog_set}; do
    (
        lang=`echo ${rtask} | awk -F'_' '{print $3}'`
        model_dir=/home/hltmbask/espnet/egs/jsalt18e2e/asr1/exp/train_mult_babel10Mult_AttCtcOut_adapt_tr_${lang}_Shinji_bs30_sgd_1e-4_${hrs}hrs_it2
        lmexpdir=/export/b15/jcho/espnet_JSALT2018/egs/jsalt18e2e/asr1/exp/train_rnnlm_2layer_bs256_mono_mlasrdict_${lang}
        ...
        ${decode_cmd} JOB=1:${nj} ${expdir}/${decode_dir}/log/decode.JOB.log \
            ...
            --model ${model_dir}/results/model.${recog_model}  \
            --model-conf ${model_dir}/results/model.conf  \
            --beam-size ${beam_size} \
            --penalty ${penalty} \
