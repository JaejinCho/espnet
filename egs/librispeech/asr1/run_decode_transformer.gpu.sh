...
train_config=conf/tuning/train_pytorch_transformer_large_ngpu4.yaml # might not be used for decoding
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
    ...
    for rtask in ${libritts_train_set} ${libritts_train_dev}; do
        decode_dir=decode_${rtask}_${recog_model}_$(basename ${decode_config%.*})_${lmtag}_gpudecoding
        ...
        --rnnlm ${lmexpdir}/${lang_model} 2>&1 | tee ${expdir}/${decode_dir}/log/decode.log
