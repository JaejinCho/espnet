...
# JJ: Edited from run.asrttsspkid_ijson.unispksampling.mem30G.sh to exclude 'text' and 'token' in 'output' dictionary after reading json files to reduce RES in training
...
expname=${expname}_spkloss_weight${spkloss_weight}_fullyunsync_mem30G_ijson_removetextNtoken
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
    ...
    ${cuda_cmd_train} --gpu ${ngpu} ${expdir}/train.log \
        tts_train_speakerid_semi_multicorpora_ijson_removetextNtoken.py \
           --num-spk ${num_spk} \
