...
# JJ: Edited from run.asrttsspkid.memoryprofile.sh
...
expname=${expname}_spkloss_weight${spkloss_weight}_fullyunsync_memoryprofile
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
    ...
    elif [ ${cuda_memsize} == 40 ];then
        cuda_cmd_train=${cuda_cmd_mem40G}
    else
        cuda_cmd_train=${cuda_cmd}
    fi
    echo "cuda_cmd_train: ${cuda_cmd_train}"
    CUDA_VISIBLE_DEVICES=${gpu_ix} fil-profile run ../../../espnet/bin/tts_train_speakerid_semi_multicorpora.py \
           --num-spk ${num_spk} \
