...
shelve_train_batchset=""
shelve_valid_batchset=""
save_shelve_dir=""
...
# the below one is the all above - "sre_tel swbd voxcelebcat_tel sre16_train_dev_cmn"
corpora_list="babel_amharic babel_assamese babel_bengali babel_cantonese babel_cebuano babel_dholuo babel_georgian babel_guarani babel_haitian babel_igbo babel_javanese babel_kazakh babel_kurmanji babel_lao babel_lithuanian babel_mongolian babel_pashto babel_swahili babel_tagalog babel_tamil babel_telugu babel_tokpisin babel_turkish babel_vietnamese babel_zulu fisher_spa sre16_eval_tr60_tgl sre16_eval_tr60_yue sre16_train_dev_ceb sre18_cmn2_train_lab"
...
expname=${expname}_spkloss_weight${spkloss_weight}_fullyunsync_mem30G_ijson_debug
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
    ...
    for cname in `echo $corpora_list`;do
        if [[ "$cname" == *"babel"* ]]; then
            tr_json_nolab+="${dumpdir}/${cname}_nolab/data.wo_textNtoken.json "
        else
            echo "LAB: $cname"
            tr_json+="${dumpdir}/${cname}_train/data.wo_textNtoken.json "
            dt_json+="${dumpdir}/${cname}_dev/data.wo_textNtoken.json "
        fi
    done
    ...
    elif [ ${cuda_memsize} == 30 ];then
        cuda_cmd_train=${cuda_cmd_mem30G}
    else
        cuda_cmd_train=${cuda_cmd}
    fi
    echo "cuda_cmd_train: ${cuda_cmd_train}"
    CUDA_VISIBLE_DEVICES=${gpu_ix} tts_train_speakerid_semi_multicorpora_ijson_shelve.py \
           --num-spk ${num_spk} \
           ...
           --train-json "${tr_json}" \
           --valid-json "${dt_json}" \
           --shelve-train-batchset "${shelve_train_batchset}" \
           --shelve-valid-batchset "${shelve_valid_batchset}" \
           --save-shelve-dir "${save_shelve_dir}" \
           --config ${train_config}
fi
