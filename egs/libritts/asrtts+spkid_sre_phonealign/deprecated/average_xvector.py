...
def convert_kaldivec2nparray(kaldivec):
	'''
	It converts the string "[ 1 2 3 ... ]" to np.array
	'''
...
def process_average(accu_uttwise, uttid_prev, fhandle_write, total_nframes=None, max_seg_nframes=1000):
	'''
	This returns one averaged x-vectors from a given list, accu_uttwise. (Optionally with random segments selection)
	accu_uttwise (list): its element is a tuple (int: seg_nframes, np.array: seg_xvec) or a np.array: seg_xvec
	'''
	sum_xvec = 0
	accu_nframes = 0
	cnt_seg = 0
	if total_nframes: # average with random segments selection
		# Shuffle segments and average until accu # frames gets to total_nframes. Set random_seed in the beginning for shuffle for deterministic results
		...
		while (len(accu_uttwise) != 0) and (accu_nframes < total_nframes):
			...
			if seg_nframes < 0: # this is when left-rotated element gets back to the front again
				...
				cnt_seg += 1
				del accu_uttwise[0]
		# (TODO-2 for update) 1) Calculate the average num_frames, 2) sort the list of 2-element tuples by the difference from average: with the absolute value within avg std range (TODO: calculate the std range), and without absolute out of the range & first with utts with more frames then, the utts with less frames
	...
	avg_emb = sum_xvec/cnt_seg
	fhandle_write.write(uttid_prev + ' [ '+' '.join(map(str, avg_emb.tolist()))+' ]\n')
...
if args.fname_in_numframes:
    # case 1: utt2num_frames are given too (select segments and only average over the segment-wise x-vectors)
    	...
    	for line1, line2 in zip(f_in_emb, f_in_nframe):
        	...
        	if uttid_cur == uttid_prev:
				...
				if len(accu_uttwise) != 0: # to filter out very first line
					process_average(accu_uttwise, uttid_prev, fhandle_write, rand_total_nframe=random.randint(1000,6000))
				accu_uttwise = [(int(seg_nframes), convert_kaldivec2nparray(seg_xvec))]
    ...
    for line in open(args.fname_in_emb):
        ...
        if uttid_cur == uttid_prev:
    		...
    		accu_uttwise = [convert_kaldivec2nparray(seg_xvec)]
    	uttid_prev = uttid_cur
