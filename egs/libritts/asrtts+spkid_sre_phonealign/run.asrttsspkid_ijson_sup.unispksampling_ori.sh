...
# Copied from run.asrttsspkid_ijson_sup.unispksampling.sh before sup-semisup
# compatible replaces the script. Now, only difference bewteen
# run.asrttsspkid_ijson_sup.unispksampling.sh and
# run.asrttsspkid_ijson.unispksampling.sh is copora_list inside each script
...
# config files
train_config=conf/train_pytorch_forwardtacotron2+spkemb_2flstm512Nusecat_fullyunsync_shufflebatching_semi_multicorpora_unispksampling.yaml # this can be also used for supervised learning
...
corpora_list="fisher_spa sre16_eval_tr60_tgl sre16_eval_tr60_yue sre16_train_dev_ceb sre18_cmn2_train_lab sre_tel swbd voxcelebcat_tel sre16_train_dev_cmn"
...
expname=${expname}_spkloss_weight${spkloss_weight}_fullyunsync_mem${mem_request}_ijson_sup
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
    ...
    ${cuda_cmd_train} --gpu ${ngpu} ${expdir}/train.log \
        tts_train_speakerid_multicorpora_ijson.py \
           --num-spk ${num_spk} \
