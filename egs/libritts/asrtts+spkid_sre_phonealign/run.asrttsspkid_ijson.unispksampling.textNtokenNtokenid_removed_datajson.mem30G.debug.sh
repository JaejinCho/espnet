...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
    ...
    for cname in `echo $corpora_list`;do
        if [[ "$cname" == *"babel"* ]]; then
            tr_json_nolab+="${dumpdir}/${cname}_nolab/data.wo_textNtokenNtokenid.json "
        else
            echo "LAB: $cname"
            tr_json+="${dumpdir}/${cname}_train/data.wo_textNtokenNtokenid.json "
            dt_json+="${dumpdir}/${cname}_dev/data.wo_textNtokenNtokenid.json "
        fi
    done
    ...
    CUDA_VISIBLE_DEVICES=${gpu_ix} tts_train_speakerid_semi_multicorpora_ijson.py \
           --num-spk ${num_spk} \
