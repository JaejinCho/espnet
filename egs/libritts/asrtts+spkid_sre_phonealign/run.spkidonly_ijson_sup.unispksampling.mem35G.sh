...
# Edited from run.asrttsspkid_ijson_sup.unispksampling.mem35G.sh
# corpora in training (nolab). ***NOTE***: This script is only supposed to be
# run from stage 4
...
# config files
train_config=conf/train_spkidonly_unsync_shufflebatching_multicorpora_unispksampling.yaml
...
expname=${expname}_ttslossw${ttsloss_weight}_spkloss_weight${spkloss_weight}_mem35G_ijson_sup
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
    ...
    ${cuda_cmd_train} --gpu ${ngpu} ${expdir}/train.log \
        tts_train_speakeridonly_temp_multicorpora_ijson.py \
           --num-spk ${num_spk} \
