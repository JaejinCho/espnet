...
# JJ: Edited from run.cleanrecon.sh
...
# config files
train_config=conf/train_pytorch_forwardtacotron2+spkemb_2flstm512Nusecat_fullyunsync_shufflebatching_semi_multicorpora_unispksampling_cleanrecon.yaml
...
corpora_list_lab="sre16_train_dev_cmn sre16_eval_tr60_tgl"
corpora_list_nolab="babel_lithuanian babel_javanese babel_telugu babel_pashto"
...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
    "stage 3: Create uttid2clean_featark & Generate  data.cleanrecon.json"
    for cname in ${corpora_list_lab}; do
        for subset in train dev; do
...
expname=${expname}_spkloss_weight${spkloss_weight}_fullyunsync_cleanrecon
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
    ...
    ${cuda_cmd_train} --gpu ${ngpu} ${expdir}/train.log \
        tts_train_speakerid_semi_multicorpora_ijson_cleanrecon.py \
           --num-spk ${num_spk} \
