...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
    # (TODO) create utt2orilink. Also, might move the process of generating data.cleanrecon.json here
    noisy_uttid2ori_uttark.py
fi
    ...
    for cname in ${corpora_list_lab} ${corpora_list_nolab}; do
        if [[ "$cname" == *"babel"* ]]; then
            ...
            if [ ! -f ${dumpdir}/${cname}_combined_nolab/data.cleanrecon.json ]; then
                ...
                local/update_json_general.sh --loc input --k featori ${dumpdir}/${cname}_combined_nolab/data.cleanrecon.json ./${dumpdir}/${cname}_combined_nolab/utt2orilink
            fi
            ...
            for subset in train dev; do
                ...
                if [ ! -f ${dumpdir}/${cname}_combined_${subset}/data.cleanrecon.json ]; then
                    ...
                    local/update_json_general.sh --loc input --k featori ${dumpdir}/${cname}_combined_${subset}/data.cleanrecon.json ./${dumpdir}/${cname}_combined_${subset}/utt2orilink
                fi
            done
