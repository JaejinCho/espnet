...
train_set_ori=/export/b11/jcho/espnet/egs/wsj/asr1/data/voxceleb1_train
train_set=voxceleb1_train_train
dev_set=voxceleb1_train_dev
...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
    ...
    for fpath in `ls data/voxceleb1_train/voxceleb1_train_{train,dev}.uttlist`;do
        dname=$(basename ${fpath} | cut -d'.' -f1) # this is same as dname=$(basename ${fpath%.*})
        subset_data_dir.sh --utt-list ${fpath} data/voxceleb1_train/ data/${dname}/
        ls -A data/${dname}/*
    done
fi
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    ...
    for subset in ${train_set} ${dev_set}; do
        # update json
        local/update_json_general.sh --loc output --k spklab ${dumpdir}/${subset}/data.json data/voxceleb1_train/voxceleb1_train.utt2spklab # utt2spklab generated same way as for utt2spkid from Nanxin's Resnet code
    done
fi
