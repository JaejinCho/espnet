...
# config files
train_config=conf/train_pytorch_tacotron2+spkemb_bs48.yaml
...
# Get only 1-80 part of the features from feats.scp (to remove pitch parts)
## recal. cmvn for fbank only
if [ ! -f data/${train_set}/cmvn.fbankonly.ark ];then
    select-feats 0-79 scp:data/${train_set}/feats.scp ark,scp:data/${train_set}/feats.fbankonly.ark,data/${train_set}/feats.fbankonly.scp
    compute-cmvn-stats scp:data/${train_set}/feats.fbankonly.scp data/${train_set}/cmvn.fbankonly.ark
fi
...
if [ ${stage} -le 6 ] && [ ${stop_stage} -ge 6 ]; then
    ...
    for subset in ${dev_set}; do
    (
        # only get fbank part from reconstructed feats
        select-feats 0-79 scp:${outdir}/${subset}/feats.scp ark,scp:${outdir}/${subset}/feats.fbankonly.ark,${outdir}/${subset}/feats.fbankonly.scp

        [ ! -e ${outdir}_denorm_16kVoceleb1Config/${subset} ] && mkdir -p ${outdir}_denorm_16kVoceleb1Config/${subset}
        apply-cmvn --norm-vars=true --reverse=true data/${train_set}/cmvn.fbankonly.ark \
            scp:${outdir}/${subset}/feats.fbankonly.scp \
            ark,scp:${outdir}_denorm_16kVoceleb1Config/${subset}/feats.fbankonly.ark,${outdir}_denorm_16kVoceleb1Config/${subset}/feats.fbankonly.scp
        convert_fbank.fbankonly.sh --nj ${nj} --cmd "${train_cmd}" \
            --fs ${fs} \
            --fmax "${fmax}" \
            --fmin "${fmin}" \
            --n_fft ${n_fft} \
            ...
            ${outdir}_denorm_16kVoceleb1Config/${subset} \
            ${outdir}_denorm_16kVoceleb1Config/${subset}/log \
            ${outdir}_denorm_16kVoceleb1Config/${subset}/wav
    ) &
    pids+=($!) # store background pids
    done
    i=0; for pid in "${pids[@]}"; do wait ${pid} || ((i++)); done
