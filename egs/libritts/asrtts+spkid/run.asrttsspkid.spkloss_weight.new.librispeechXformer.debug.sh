...
# config files
train_config=conf/train_pytorch_tacotron2+spkemb.debug.yaml
...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
    ...
    for subset in ${train_set} ${dev_set};do
        if [ ! -d data/${subset}_Xformer ];then
            ...
            awk -F' ' '$2!=""' /export/b18/jcho/espnet_asr_transformer_final/egs/librispeech/asr1/data/${subset}/text.asr > data/${subset}_Xformer/text # changed from the above to this (to remove empty transcription utterances)
            fix_data_dir.sh data/${subset}_Xformer
        else
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    ...
    for subset in ${train_set} ${dev_set}; do
        # list for removing utterances that have empty ASR transcriptions
        awk -F' ' '{print NF}' /export/b18/jcho/espnet_asr_transformer_final/egs/librispeech/asr1/data/${subset}/text.asr | grep -n -w 1 | awk -F':' '{print $1}' > ${dumpdir}/${subset}/list_Xformer_emptylines.txt
        acc=""
        for ix in `cat ${dumpdir}/${subset}/list_Xformer_emptylines.txt`;do
            acc="${acc};${ix}d"
        done
        acc=`echo ${acc} | cut -c 2-`
        sed -e "${acc}" ${dumpdir}/${subset}/feats.scp > ${dumpdir}/${subset}_Xformer/feats.scp
        # make json labels
        data2json.sh --feat ${dumpdir}/${subset}_Xformer/feats.scp data/${subset}_Xformer ${dict} > ${dumpdir}/${subset}_Xformer/data.json
        local/update_json_general.sh --loc output --k spklab ${dumpdir}/${subset}_Xformer/data.json data/train_clean_460/train_trainNdev.utt2spklab # utt2spklab generated same way as for utt2spkid from Nanxin's Resnet code
    done
fi
num_spk=`cat data/${train_set}_Xformer/spk2utt | cut -d'_' -f1 | sort -u | wc -l` # should be 1k for now
if [ ! $num_spk -eq 999 ];then
    echo "num speaker: ${num_spk} is supposed to be 999." && exit 1 # Since some speakers are filtered out due to empty transcription generated from ASR
fi
