...
# JJ: edited from run.asrttsspkid.spkloss_weight.voxceleb1_filtered_bs48.librispeechXformer.sh
...
train_set_ori=/export/b18/jcho/espnet_asr_transformer_final/egs/librispeech/asr1/data/voxceleb1_train_filtered
train_set=voxceleb1_train_filtered_train_librispeechXformerASR
dev_set=voxceleb1_train_filtered_dev_librispeechXformerASR
...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
    ...
    if [ ! -d data/voxceleb1_train_filtered_librispeechXformerASR ];then
        mkdir -p data && cp -r ${train_set_ori} data/voxceleb1_train_filtered_librispeechXformerASR
    fi
    ...
    for fpath in `ls data/voxceleb1_train_filtered_librispeechXformerASR/voxceleb1_train_filtered_{train,dev}.uttlist`;do
        dname=$(basename ${fpath} | cut -d'.' -f1) # this is same as dname=$(basename ${fpath%.*})
        subset_data_dir.sh --utt-list ${fpath} data/voxceleb1_train_filtered_librispeechXformerASR/ data/${dname}_librispeechXformerASR/
        ls -A data/${dname}/*
    done
fi
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    ...
    for subset in ${train_set} ${dev_set}; do
        # update json
        local/update_json_general.sh --loc output --k spklab ${dumpdir}/${subset}/data.json data/voxceleb1_train_filtered_librispeechXformerASR/voxceleb1_train_filtered.utt2spklab # utt2spklab generated same way as for utt2spkid from Nanxin's Resnet code
    done
fi
...
expname=${expname}_spkloss_weight${spkloss_weight}_consloss_detach_weight${consloss_weight}_bs${batch_size}
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
    ...
    ${cuda_cmd} --gpu ${ngpu} ${expdir}/train.log \
        tts_train_speakerid_consistency.py \
           --model-module espnet.nets.pytorch_backend.e2e_tts_tacotron2_speakerid_consistency_detach:Tacotron2 \
           --num-spk ${num_spk} \
           --train-spkid-extractor True \
           --train-spk-embed-dim 400 \
           --spkloss-weight ${spkloss_weight} \
           --consloss-weight ${consloss_weight} \
           --backend ${backend} \
           --ngpu ${ngpu} \
