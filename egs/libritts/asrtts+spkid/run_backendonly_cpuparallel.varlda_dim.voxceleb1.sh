...
# JJ: Copied from ../asrtts+spkid_voxceleb1_kaldiasr_phonealign_rf3/run_backendonly_cpuparallel.varlda_dim.voxceleb1.sh
...
if [ ${stage} -le 6 ] && [ ${stop_stage} -ge 6 ]; then
  ...
  if [ ! -f dump/voxceleb1_train_filtered_trainNdev/emb_mean.vec.${embname} ]; then
  ...
  lda_dim=${lda_dim}
  $train_cmd dump/voxceleb1_train_filtered_trainNdev/log/lda.lda_dim${lda_dim}.${embname}.log \
    ivector-compute-lda --total-covariance-factor=0.0 --dim=$lda_dim \
    "ark:ivector-subtract-global-mean ark:dump/voxceleb1_train_filtered_trainNdev/emb.${embname} ark:- |" \
    ark:data/voxceleb1_train_filtered_trainNdev/utt2spk dump/voxceleb1_train_filtered_trainNdev/emb_transform.mat.lda_dim${lda_dim}.${embname} || exit 1;

  # Train an out-of-domain PLDA model.
  $train_cmd dump/voxceleb1_train_filtered_trainNdev/log/plda.lda_dim${lda_dim}.${embname}.log \
    ivector-compute-plda ark:data/voxceleb1_train_filtered_trainNdev/spk2utt \
    "ark:ivector-subtract-global-mean ark:dump/voxceleb1_train_filtered_trainNdev/emb.${embname} ark:- | transform-vec dump/voxceleb1_train_filtered_trainNdev/emb_transform.mat.lda_dim${lda_dim}.${embname} ark:- ark:- | ivector-normalize-length ark:-  ark:- |" \
    dump/voxceleb1_train_filtered_trainNdev/plda.lda_dim${lda_dim}.${embname} || exit 1;
fi
# stage 9
if [ ${stage} -le 7 ] && [ ${stop_stage} -ge 7 ]; then
  echo "stage 7: Calculate score"
  # Get results using the out-of-domain PLDA model.
  $train_cmd exp/scores/log/spkid_scoring_lda_dim${lda_dim}.${embname}.log \
    ivector-plda-scoring --normalize-length=true \
    "ivector-copy-plda --smoothing=0.0 dump/voxceleb1_train_filtered_trainNdev/plda.lda_dim${lda_dim}.${embname} - |" \
    "ark:ivector-subtract-global-mean dump/voxceleb1_train_filtered_trainNdev/emb_mean.vec.${embname} ark:dump/${eval_set}/emb.${embname} ark:- | transform-vec dump/voxceleb1_train_filtered_trainNdev/emb_transform.mat.lda_dim${lda_dim}.${embname} ark:- ark:- | ivector-normalize-length ark:- ark:- |" \
    "ark:ivector-subtract-global-mean dump/voxceleb1_train_filtered_trainNdev/emb_mean.vec.${embname} ark:dump/${eval_set}/emb.${embname} ark:- | transform-vec dump/voxceleb1_train_filtered_trainNdev/emb_transform.mat.lda_dim${lda_dim}.${embname} ark:- ark:- | ivector-normalize-length ark:- ark:- |" \
    "cat data/${eval_set}/trials | cut -d\  --fields=1,2 |" exp/scores_voxceleb1_${eval_set}_lda_dim${lda_dim}.embname${embname} || exit 1;
fi
# stage 10
if [ ${stage} -le 8 ] && [ ${stop_stage} -ge 8 ]; then
  echo "stage 8: EER & MinDCF"
  qsub -cwd -o log/qsub.run.eer_mindcf.voxceleb1.lda_dim${lda_dim}.${embname}.log -e log/qsub.run.eer_mindcf.voxceleb1.lda_dim${lda_dim}.${embname}.log -l mem_free=200G,ram_free=200G run.eer_mindcf.varlda_dim.voxceleb1.sh --lda_dim ${lda_dim} --eval_set ${eval_set} --embname ${embname}
fi
### JJ - end
