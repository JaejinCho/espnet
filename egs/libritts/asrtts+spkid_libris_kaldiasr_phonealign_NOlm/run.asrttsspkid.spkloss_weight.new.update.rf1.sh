...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
    ...
    for subset in ${train_set} ${dev_set};do
        awk '{$1=$1;print}' /export/b11/jcho/kaldi_20190316/egs/librispeech/s5/exp/chain_cleaned/tdnn_1d_sp/decode_train_clean_460_frame_subsampling_factor_3_NOlm/phone.perframe.expand.sym.txt | rev | cut -d' ' -f3- | rev > data/${subset}/text # JJ: remove last 2 phonemes in each utterance to match # frames with # frames in fbank features
