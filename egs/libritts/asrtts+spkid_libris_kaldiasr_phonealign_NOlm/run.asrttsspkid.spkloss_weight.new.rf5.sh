...
# config files
train_config=conf/train_pytorch_tacotron2+spkemb_rf5.yaml
...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
    ...
    for subset in ${train_set} ${dev_set};do
        cp /export/b18/jcho/espnet_asr_transformer_final/egs/librispeech/asr1/data/${subset}/text.asr data/${subset}/text
        # remove the lines where text is empty and fix the directory
        mv data/${subset}/text data/${subset}/.text && awk '{if(!(NF==1))print $0}' data/${subset}/.text > data/${subset}/text
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    ...
    for subset in ${train_set} ${dev_set}; do
        # update json
        local/update_json_general.sh --loc output --k spklab ${dumpdir}/${subset}/data.json data/train_clean_460/train_trainNdev.utt2spklab_NOspk2085 # utt2spklab generated same way as for utt2spkid from Nanxin's Resnet code
    done
fi
num_spk=`cat data/${train_set}/spk2utt | cut -d'_' -f1 | sort -u | wc -l` # should be 1k for now
echo "num speaker: ${num_spk} is reduced from 1k."
