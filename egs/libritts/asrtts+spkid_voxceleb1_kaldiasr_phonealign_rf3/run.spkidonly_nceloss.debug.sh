...
# config files
#train_config=conf/train_pytorch_tacotron2+spkemb_spkidonly_nceloss_improve_shufflebatching_bs76.yaml
train_config=conf/train_pytorch_tacotron2+spkemb_spkidonly_nceloss_improve_shufflebatching_bs76_unisamplespk.yaml
...
expname=${expname}_speakeridonly_ttslossw${ttsloss_weight}_spkloss_weight${spkloss_weight}_final_nceloss_weight${nceloss_weight}_unsync200to400random_debug
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
    ...
    dt_json=${feat_dt_dir}/data.json
        CUDA_VISIBLE_DEVICES=${gpu_ix} tts_train_speakeridonly_temp.py \
           --num-spk ${num_spk} \
