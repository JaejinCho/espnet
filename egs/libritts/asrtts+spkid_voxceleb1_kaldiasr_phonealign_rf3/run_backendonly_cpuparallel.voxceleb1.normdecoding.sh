...
# Edited from run_backendonly_cpuparallel.voxceleb1.sh to include emb normalization
...
if [[ model.loss.best == ${model} ]]; then
    embname=`basename ${expdir}`_embnormalized
else
    embname=`basename ${expdir}`_${model}_embnormalized
fi
