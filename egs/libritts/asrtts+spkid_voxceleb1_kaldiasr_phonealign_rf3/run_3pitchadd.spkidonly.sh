...
# JJ: Edited from run.spkidonly.sh
# JJ: Edited from
# run.asrttsspkid.spkloss_weight.new.update.rf1.forwardtacotron2.sh to follow
# stage 4 of /export/b18/jcho/espnet3/egs/libritts/asrtts+spkid/run.spkidonly.final.voxceleb1.chunks.shufflebatching.sh
...
# exp tag
tag="3pitchadd" # tag for managing experiments.
...
for subset in ${train_set} ${dev_set} ${eval_set};do
    # data prep.
    mkdir -p data/${subset}
    cp data/${subset%_3pitchadd}/{text,utt2spk} data/${subset}/ # fr data2json_kaldiali.sh
    # dump prep.
    pitchdir=../asrtts+spkid
    paste-feats --length-tolerance=1 scp:dump/${subset%_3pitchadd}/feats.scp "ark:select-feats 80-82 scp:${pitchdir}/dump/${subset%_3pitchadd}/feats.scp ark:-|" ark,scp:dump/${subset}/feats.ark,dump/${subset}/feats.scp
done

dict=data/lang_1char/${train_set%_3pitchadd}_units.txt
