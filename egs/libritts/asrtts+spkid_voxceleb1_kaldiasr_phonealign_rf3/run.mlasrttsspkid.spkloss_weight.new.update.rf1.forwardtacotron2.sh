...
# JJ: Edited from
# run.asrttsspkid.spkloss_weight.new.update.rf1.forwardtacotron2.sh (only
# change the text file and the directory name)
...
train_set=voxceleb1_train_filtered_train_mlasr
dev_set=voxceleb1_train_filtered_dev_mlasr

if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
    echo "stage 0: Data preparation (copy data directories)"
    mkdir -p data
    for subset in ${train_set} ${dev_set};do
        #cp -r ${feat_dir}/data/${subset} data/${subset}
        cp -r data/${subset%_mlasr} data/${subset}
        ls -A data/${subset}
    done
    for subset in ${train_set} ${dev_set};do
        cp /export/b11/jcho/kaldi_20190316/egs/babel/masr_grapheme/exp/chain/tdnn1h_d_sp/decode_voxceleb1_train_filtered_frame_subsampling_factor_3/phone.perframe.sym.txt data/${subset}/text # (TODO: Check this) phone ali seq * 3 - (2, 3 or 4) = # acoustic seq
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    ...
    echo "stage 1: Feature Generation (Skip this since only text file is changed)"
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    ...
    # JJ add - start
    cp /export/b11/jcho/kaldi_20190316/egs/babel/masr_grapheme/exp/chain/tdnn1h_d_sp/phones.txt ${dict}
    sed -e '1 s:<eps> 0:<unk> 0:' ${dict} | awk -F' ' '{$2=$2+1;print}' OFS=' ' > .tmpdict && mv .tmpdict ${dict} # (TODO: Check this part)
    # JJ add - end
    wc -l ${dict}

    # make json labels
    data2json_kaldiali.sh --kaldiali true --feat ${feat_tr_dir%_mlasr}/feats.scp \
         data/${train_set} ${dict} > ${feat_tr_dir}/data.json
    data2json_kaldiali.sh --kaldiali true --feat ${feat_dt_dir%_mlasr}/feats.scp \
