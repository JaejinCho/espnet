...
trial_path=dump/${eval_set}/trials # for now it is fixed all the time
score_path=exp/scores_voxceleb1_${eval_set}_embname${embname} # path for the calculated scores given trials
python /export/b17/janto/SRE18/v1.8k/scoring_software/scoring.py ${trial_path} ${score_path} 2>&1 | tee log/run.eer_mindcf.${embname}.${eval_set}.log
