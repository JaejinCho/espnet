...
if [ ! -f data/${eval_set}/trials ]; then
    python convert_trials.py data/voxceleb1_test/utt2num_frames dump/${eval_set}/utt2num_frames data/voxceleb1_test/trials dump/${eval_set}/trials
fi
...
if [ ${stage} -le 7 ] && [ ${stop_stage} -ge 7 ]; then
  ...
  $train_cmd exp/scores/log/spkid_scoring_${embname}.${eval_set}.log \
    ...
    "ark:ivector-subtract-global-mean dump/voxceleb1_train_filtered_trainNdev/emb_mean.vec.${embname} ark:dump/${eval_set}/emb.${embname} ark:- | transform-vec dump/voxceleb1_train_filtered_trainNdev/emb_transform.mat.${embname} ark:- ark:- | ivector-normalize-length ark:- ark:- |" \
    "cat dump/${eval_set}/trials | cut -d\  --fields=1,2 |" exp/scores_voxceleb1_${eval_set}_embname${embname} || exit 1;
fi
# stage 10
if [ ${stage} -le 8 ] && [ ${stop_stage} -ge 8 ]; then
  echo "stage 8: EER & MinDCF"
  qsub -cwd -o log/qsub.run.eer_mindcf.voxceleb1.${embname}.${eval_set}.log -e log/qsub.run.eer_mindcf.voxceleb1.${embname}.${eval_set}.log -l mem_free=200G,ram_free=200G run.eer_mindcf.voxceleb1.shortseg.sh --eval_set ${eval_set} --embname ${embname}
fi
### JJ - end
