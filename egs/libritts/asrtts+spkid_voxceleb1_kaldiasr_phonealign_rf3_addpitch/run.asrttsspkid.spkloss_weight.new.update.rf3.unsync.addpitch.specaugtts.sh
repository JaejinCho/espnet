...
# JJ: Edited from run.asrttsspkid.spkloss_weight.new.update.rf3.unsync.addpitch.sh
...
# config files
preprocess_config=conf/specaug.yaml # spec data aug.
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    ...
    for subset in ${train_set} ${dev_set}; do
        ...
        cp data/${subset}/pitch.scp data/${subset}/.pitch.txt # copy to have .txt extension to use local/update_json_general.sh properly
        local/update_json_general.sh --loc input --k pitch ${dumpdir}/${subset}/data.json data/${subset}/.pitch.txt # featdir: some dump dir
    done
fi
...
expname=${expname}_spkloss_weight${spkloss_weight}_unsync_addpitch
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
    ...
    ${cuda_cmd_mem5G} --gpu ${ngpu} ${expdir}/train.log \
        tts_train_speakerid_addpitch_specaugtts.py \
           --model-module espnet.nets.pytorch_backend.e2e_tts_tacotron2_speakerid_update_unsync_addpitch_specaugtts:Tacotron2 \
           --num-spk ${num_spk} \
