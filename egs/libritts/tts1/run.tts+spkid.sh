...
train_spktrain_set=train_clean_460_spktrain_0.9
train_spkcv_set=train_clean_460_spkcv_0.1
dev_set=dev_clean
eval_set=test_clean
...
#feat_tr_dir=${dumpdir}/${train_spktrain_set}; mkdir -p ${feat_tr_dir}
#feat_dt_dir=${dumpdir}/${dev_set}; mkdir -p ${feat_dt_dir}
feat_tr_dir=${dumpdir}/${train_spktrain_set}; mkdir -p ${feat_tr_dir}
feat_dt_dir=${dumpdir}/${train_spkcv_set}; mkdir -p ${feat_dt_dir}
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    ...
    awk '!($2 in a){a[$2]=0} {a[$2]+=1} END{for(i in a) print i, a[i]}' data/${train_set}/utt2spk > data/${train_set}/spk2num
    ## divide to train and cv (put into train all the utterances from a speaker where its #utts is less than 10)
    ### generate uttlist
    awk -v data_dir=data/${train_set}/ 'BEGIN{srand(1);} FNR==NR{a[$1]=$2; next} {if(a[$2]<10)print $1 >> data_dir"train.spkid.uttlist";else{if(rand()<=0.1)print $1 >> data_dir"cv.spkid.uttlist";else print $1 >> data_dir"train.spkid.uttlist"}}' data/${train_set}/spk2num data/${train_set}/utt2spk
    ### Divide data/${train_set}/. NOT divide dump/${train_set}/feats.scp (since cmvn.ark is calculated again)
    utils/subset_data_dir.sh --utt-list data/${train_set}/train.spkid.uttlist data/${train_set} data/${train_spktrain_set}
    utils/subset_data_dir.sh --utt-list data/${train_set}/cv.spkid.uttlist data/${train_set} data/${train_spkcv_set}
    ...
    compute-cmvn-stats scp:data/${train_spktrain_set}/feats.scp data/${train_spktrain_set}/cmvn.ark

    # dump features for training
    dump.sh --cmd "$train_cmd" --nj ${nj} --do_delta false \
        data/${train_spktrain_set}/feats.scp data/${train_spktrain_set}/cmvn.ark exp/dump_feats/train ${feat_tr_dir}
    dump.sh --cmd "$train_cmd" --nj ${nj} --do_delta false \
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    ...
    awk 'BEGIN{s=0;}{if(!($2 in a)){a[$2]=s;s+=1;}print $1,a[$2]}' data/${train_set}/utt2spk > data/${train_set}/utt2spklab # here data/${train_set}/utt2spk == data/${train_spktrain_set}/utt2spk
    for subset in ${train_spktrain_set} ${train_spkcv_set}; do
        # update json
        local/update_json_general.sh --loc output -k spklab ${dumpdir}/${subset}/data.json data/${train_set}/utt2spklab # utt2spklab generated same way as for utt2spkid from Nanxin's Resnet code
    done
fi
