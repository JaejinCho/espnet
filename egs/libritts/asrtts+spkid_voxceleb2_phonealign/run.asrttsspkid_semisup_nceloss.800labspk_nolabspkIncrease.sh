...
# JJ: Edited from run.asrttsspkid_semisup.800labspk_nolabspkIncrease.sh
...
# config files
train_config=conf/train_pytorch_forwardtacotron2+spkemb_2flstm512Nusecat_fullyunsync_shufflebatching_nceloss_semi.yaml
...
# Speaker id related
spkloss_weight=0.03
labspk_num=800 # this is # spks for labeled samples in semi-supervised training
nolabspk_num='' # this is # spks for nolabeled samples in semi-supervised training
...
train_set_nolab_nolab=${whole_train}_${labspk_num}spk_stack_nolab_${nolabspk_num}_nolab
...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
   ...
   #     cp -r /export/b13/jcho/espnet_v3/tools/kaldi/egs/voxceleb/v2/data/${whole_train} data/
    ...
    # Extract ${nolabspk_num} nolabspk subsets
    if [ ! -f data/${train_set_nolab}/spklist ]; then
        awk -F' ' '{print $1}' data/${train_set_nolab}/spk2utt | sort -u | shuf > data/${train_set_nolab}/spklist
        echo "# speakers in original nolab: $(wc -l data/${train_set_nolab}/spklist)" # expected to be 5314 for data/${train_set_nolab} when labspk_num=800
    fi
    # nolab subset
    if [ ! -d data/${train_set_nolab_nolab} ]; then
        subset_data_dir.sh --spk-list <(head -n${nolabspk_num} data/${train_set_nolab}/spklist) data/${train_set_nolab} data/${train_set_nolab_nolab}
    fi
    # 2. Divide data/${train_set_lab} into data/${train_set,dev_set} (*** This should be done already from another run*.sh script ***)
...
feat_tr_dir_nolab_nolab=${dumpdir}/${train_set_nolab_nolab}; mkdir -p ${feat_tr_dir_nolab_nolab}
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    ...
    filter_scp.pl data/${train_set_nolab_nolab}/utt2spk ${dumpdir}/${train_set_nolab}/feats.scp > ${feat_tr_dir_nolab_nolab}/feats.scp
    cp data/${train_set_nolab_nolab}/utt2spk ${feat_tr_dir_nolab_nolab}/ && utils/fix_data_dir.sh ${feat_tr_dir_nolab_nolab}/
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    ...
    # *** Below block is supposed to be done in anothre run*.sh script
    ...
    data2json_kaldiali.sh --kaldiali true --feat ${feat_tr_dir_nolab_nolab}/feats.scp \
         data/${train_set_nolab_nolab} ${dict} > ${feat_tr_dir_nolab_nolab}/data.json
    ...
    awk '{print $1, -1}' ./data/${train_set_nolab_nolab}/utt2spk > ./data/${train_set_nolab_nolab}/utt2spklab
    local/update_json_general.sh --loc output --k spklab ${dumpdir}/${train_set_nolab_nolab}/data.json ./data/${train_set_nolab_nolab}/utt2spklab
fi

num_spk=`cat data/${train_set}/spk2utt | wc -l`
num_spk_nolab_nolab=`cat data/${train_set_nolab_nolab}/spk2utt | wc -l`
echo The number of speakers HAVING LABELS for spkid training: ${num_spk} # ${labspk_num}
echo The number of speakers NOT HAVING LABELS for spkid training: ${num_spk_nolab_nolab} # ${nolabspk_num}

if [ -z ${tag} ]; then
    expname=${train_set}_${nolabspk_num}nolabspk_$(basename ${train_config%.*})
else
    expname=${train_set}_${nolabspk_num}nolabspk_$(basename ${train_config%.*})_${tag}
fi
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
    ...
    tr_json_nolab_nolab=${feat_tr_dir_nolab_nolab}/data.json
    ...
    ${cuda_cmd_train} --gpu ${ngpu} ${expdir}/train.log \
        tts_train_speakerid_nceloss_semi.py \
           --num-spk ${num_spk} \
           ...
           --seed ${seed} \
           --resume ${resume} \
           --train-json-nolab ${tr_json_nolab_nolab} \
           --train-json ${tr_json} \
           --valid-json ${dt_json} \
           --config ${train_config}
fi
