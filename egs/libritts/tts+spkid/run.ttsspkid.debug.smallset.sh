...
expname=${expname}_debug
expdir=exp/${expname}
mkdir -p ${expdir}
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
    echo "stage 4: Text-to-speech model training"
    #tr_json=${feat_tr_dir}/data.json
    tr_json=dump_debug/train_clean_460_spktrain_0.9/data.json
    #dt_json=${feat_dt_dir}/data.json
    dt_json=dump_debug/train_clean_460_spkcv_0.1/data.json
...
if [ ${stage} -le 7 ] && [ ${stop_stage} -ge 7 ]; then
    ...
    speakerid_eval_classification.py \
        ...
        --ngpu ${ngpu} \
        --verbose ${verbose} \
        --json dump_debug/train_clean_460_spkcv_0.1/data.json \
        --model ${expdir}/results/${model} \
        --batch-size 256
fi
