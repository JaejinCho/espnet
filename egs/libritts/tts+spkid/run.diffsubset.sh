...
dumpdir=dump_ttsonly # directory to dump full features
...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
    ...
    for name in ${train_set} ${dev_set}; do
        local/update_json.sh ${dumpdir}/${name}/data.json ${nnet_dir}/xvectors_${train_set_ori}/xvector.scp
    done
    local/update_json.sh ${dumpdir}/${eval_set}/data.json ${nnet_dir}/xvectors_${eval_set}/xvector.scp
fi
