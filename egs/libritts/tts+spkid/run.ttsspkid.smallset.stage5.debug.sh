...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    ...
    utils/combine_data.sh data/${train_set_ori}_org data/train_clean_100 data/train_clean_360
    # remove utt having more than 3000 frames
    # remove utt having more than 400 characters
    remove_longshortdata.sh --maxframes 3000 --maxchars 400 data/${train_set_ori}_org data/${train_set_ori}

    # JJ: spkid
    ## get utt2spklab: uttid and spkid (in integer) as key and value pair
    awk 'BEGIN{s=0;}{if(!($2 in a)){a[$2]=s;s+=1;}print $1,a[$2]}' data/${train_set_ori}/utt2spk > data/${train_set_ori}/utt2spklab # here data/${train_set_ori}/utt2spk == data/${train_set}/utt2spk
    ## get spk2num: spkid and #utt as key and value pair
    awk '!($2 in a){a[$2]=0} {a[$2]+=1} END{for(i in a) print i, a[i]}' data/${train_set_ori}/utt2spk > data/${train_set_ori}/spk2num
    ## divide train_set_ori to train_set and dev_set subests (assign to train_set all the utterances from a speaker where its #utts is less than 10)
    ### generate uttlist
    awk -v data_dir=data/${train_set_ori}/ 'BEGIN{srand(1);} FNR==NR{a[$1]=$2; next} {if(a[$2]<10)print $1 >> data_dir"train.spkid.uttlist";else{if(rand()<=0.1)print $1 >> data_dir"cv.spkid.uttlist";else print $1 >> data_dir"train.spkid.uttlist"}}' data/${train_set_ori}/spk2num data/${train_set_ori}/utt2spk
    ### Divide data/${train_set_ori}/. NOT divide dump/${train_set_ori}/feats.scp (since cmvn.ark is calculated again)
    utils/subset_data_dir.sh --utt-list data/${train_set_ori}/train.spkid.uttlist data/${train_set_ori} data/${train_set}
    utils/subset_data_dir.sh --utt-list data/${train_set_ori}/cv.spkid.uttlist data/${train_set_ori} data/${dev_set}
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    ...
    for subset in ${train_set} ${dev_set}; do
        # update json
        local/update_json_general.sh --loc output --k spklab ${dumpdir}/${subset}/data.json data/${train_set_ori}/utt2spklab # utt2spklab generated same way as for utt2spkid from Nanxin's Resnet code
    done
fi
...
expname=${expname}_smallset
expdir=exp/${expname}
mkdir -p ${expdir}

for subdir in `ls dump/`;do
    mkdir -p ${dumpdir}/${subdir}
    python extract100_fordebug.py dump/${subdir}/data.json --num-example 100 > ${dumpdir}/${subdir}/data.json
done
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
    ...
    for subset in ${dev_set} ${eval_set}; do
        ...
        # decode in parallel
        # JJ: Assuming the data were split and use one piece of them only for
        # debug
        #${train_cmd} JOB=1:${nj} ${outdir}/${subset}/log/decode.JOB.log \
            tts_decode_speakerid.py \
                ...
                --ngpu 0 \
                --verbose ${verbose} \
                --out ${outdir}/${subset}/feats.1 \
                --json ${outdir}/${subset}/split${nj}utt/data.1.json \
