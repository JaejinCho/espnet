...
# /export/b18/jcho/espnet3/egs/libritts/asrtts+spkid_libris_kaldiasr_phonealign/run.asrttsspkid.spkloss_weight.new.update.rf1.sh
...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
    ...
    for subset in ${train_set} ${dev_set};do
        awk '{$1=$1;print}' /export/b11/jcho/kaldi_20190316/egs/librispeech/s5/exp/chain_cleaned/tdnn_1d_sp/decode_voxceleb1_all_frame_subsampling_factor_3/phone.perframe.expand.sym.txt | rev | cut -d' ' -f2- | rev > data/${subset}/text # JJ: remove last 1 phonemes in each utterance to match # frames with # frames in fbank features
