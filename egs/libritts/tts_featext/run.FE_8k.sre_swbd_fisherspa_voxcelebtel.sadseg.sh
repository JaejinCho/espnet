...
data_list="sre_tel sre16_eval_tr60_tgl sre16_eval_tr60_yue sre16_train_dev_ceb sre16_train_dev_cmn sre18_cmn2_train_lab swbd fisher_spa voxcelebcat_tel"
...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
    ...
    (
        ...
        fix_data_dir.sh data/${dataname} # detect_speech_activity*.sh also include this step
    ) &
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    ...
    for dataname in ${data_list}; do
    (
        if [ ! -s data/${dataname}/feats.scp ]; then
            temp_nj=$(wc -l data/${dataname}/spk2utt | awk '{print $1}')
            ...
            echo "WARNING: There exist feats.scp already under data/${dataname} so skipping the feature extraction"
        fi
    ) &
    pids+=($!) # store background pids
    done
