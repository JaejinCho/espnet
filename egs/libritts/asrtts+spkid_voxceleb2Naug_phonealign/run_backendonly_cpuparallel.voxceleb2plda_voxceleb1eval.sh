...
if [ ${stage} -le 6 ] && [ ${stop_stage} -ge 6 ]; then
  ...
  if [ ! -d dump/voxceleb2_trainNdev ]; then
    mkdir -p dump/voxceleb2_trainNdev
  fi
  cat dump/voxceleb2_{train,dev}/emb.${embname} > dump/voxceleb2_trainNdev/emb.${embname}
  rm dump/voxceleb2_{train,dev}/emb.${embname}

  # to combine utt2spk and spk2utt
  if [ ! -d data/voxceleb2_trainNdev ]; then
    combine_data.sh data/voxceleb2_trainNdev data/voxceleb2_train data/voxceleb2_dev
  fi


  $train_cmd dump/voxceleb2_trainNdev/log/compute_emb_mean_${embname}.log \
      ivector-mean ark:dump/voxceleb2_trainNdev/emb.${embname} \
      dump/voxceleb2_trainNdev/emb_mean.vec.${embname} || exit 1;

  # This script uses LDA to decrease the dimensionality prior to PLDA.
  lda_dim=150
  $train_cmd dump/voxceleb2_trainNdev/log/lda.${embname}.log \
    ivector-compute-lda --total-covariance-factor=0.0 --dim=$lda_dim \
    "ark:ivector-subtract-global-mean ark:dump/voxceleb2_trainNdev/emb.${embname} ark:- |" \
    ark:data/voxceleb2_trainNdev/utt2spk dump/voxceleb2_trainNdev/emb_transform.mat.${embname} || exit 1;

  # Train an out-of-domain PLDA model.
  $train_cmd dump/voxceleb2_trainNdev/log/plda.${embname}.log \
    ivector-compute-plda ark:data/voxceleb2_trainNdev/spk2utt \
    "ark:ivector-subtract-global-mean ark:dump/voxceleb2_trainNdev/emb.${embname} ark:- | transform-vec dump/voxceleb2_trainNdev/emb_transform.mat.${embname} ark:- ark:- | ivector-normalize-length ark:-  ark:- |" \
    dump/voxceleb2_trainNdev/plda.${embname} || exit 1;
fi
# stage 9
if [ ${stage} -le 7 ] && [ ${stop_stage} -ge 7 ]; then
  ...
  $train_cmd exp/scores/log/spkid_scoring_${embname}.log \
    ...
    "ark:ivector-subtract-global-mean dump/voxceleb2_trainNdev/emb_mean.vec.${embname} ark:dump/${eval_set}/emb.${embname} ark:- | transform-vec dump/voxceleb2_trainNdev/emb_transform.mat.${embname} ark:- ark:- | ivector-normalize-length ark:- ark:- |" \
    "ark:ivector-subtract-global-mean dump/voxceleb2_trainNdev/emb_mean.vec.${embname} ark:dump/${eval_set}/emb.${embname} ark:- | transform-vec dump/voxceleb2_trainNdev/emb_transform.mat.${embname} ark:- ark:- | ivector-normalize-length ark:- ark:- |" \
