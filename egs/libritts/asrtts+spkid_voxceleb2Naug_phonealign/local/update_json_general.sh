...
if [ $# != 2 ]; then
    ...
    echo "e.g. : $0 local/update_json.sh dump/train/data.json exp/xvector_nnet_1a//xvectors_train/xvector.scp"
    echo "e.g. : $0 local/update_json.sh --loc output -k spklab dump/train/data.json data/train/utt2spkint \
            This usage adds key (with -k option) and value pairs to <json>. The values (the 2nd column in <data>) are added to the corresponding uttids (the 1nd column in <data>)"
    echo "Options: "
    echo "  --loc <input|output> # location when adding <data> to <json> "
    echo "  --k <e.g. spkint> # k used as a key for the actual data value."
    exit 1;
fi
