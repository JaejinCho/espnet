...
dumpdir=dump_dir # directory to dump full features
...
expname=${expname}_spkloss_weight${spkloss_weight}_nceloss_weight${nceloss_weight}_fullyunsync_cleanrecon_debug # cleanrecon means reconstructing clean version of the input by TTS
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
    ...
    tr_json=dump_debug/train/data.cleanrecon.json
    dt_json=dump_debug/dev/data.cleanrecon.json
    echo "tr_json: ${tr_json}"
    echo "dt_json: ${dt_json}"
    CUDA_VISIBLE_DEVICES=${gpu_ix} tts_train_speakerid_cleanrecon.py \
           --num-spk ${num_spk} \
