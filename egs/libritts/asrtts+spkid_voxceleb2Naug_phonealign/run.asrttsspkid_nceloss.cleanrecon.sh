...
# JJ: Edited from run.asrttsspkid.cleanrecon.sh
# This script assumes run.asrttsspkid.sh data preparation parts (before stage4)
# were already run
...
# config files
train_config=conf/train_pytorch_forwardtacotron2+spkemb_2flstm512Nusecat_nceloss_fullyunsync_shufflebatching_cleanrecon_bs56.yaml
...
expname=${expname}_spkloss_weight${spkloss_weight}_nceloss_weight${nceloss_weight}_fullyunsync_cleanrecon # cleanrecon means reconstructing clean version of the input by TTS
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
    ...
    for subset in ${train_set} ${dev_set}; do
        # update json
        if [ ! -f ${dumpdir}/${subset}/data.cleanrecon.json ]; then
            cp ${dumpdir}/${subset}/data.json ${dumpdir}/${subset}/data.cleanrecon.json
            local/update_json_general.sh --loc input --k featori ${dumpdir}/${subset}/data.cleanrecon.json ./dump/${subset}/utt2orilink
        fi
    done

    tr_json=${feat_tr_dir}/data.cleanrecon.json
    dt_json=${feat_dt_dir}/data.cleanrecon.json
    ...
    ${cuda_cmd_train} --gpu ${ngpu} ${expdir}/train.log \
        tts_train_speakerid_cleanrecon.py \
           --num-spk ${num_spk} \
