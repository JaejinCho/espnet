...
cuda_memsize=5 # this means 5. The other option is 5
...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
    ...
    # 0. Copy needed data
    if [ ! -d data/voxceleb2Naug ]; then
        f_uttlist=/export/b13/jcho/espnet_v3/tools/kaldi/egs/voxceleb/v2/data/voxceleb2/uttlist
        aug_uttlist.py ${f_uttlist} ${f_uttlist}_aug
        f_voxcelebtraincombined=/export/b18/jcho/espnet3/egs/libritts/asrtts+spkid_voxcelebNaug_kaldiasr_phonealign_rf3/data/voxceleb_train_combined # This data dir already includes text file as phone alignment
        subset_data_dir.sh --utt-list ${f_uttlist}_aug ${f_voxcelebtraincombined} data/voxceleb2Naug
    fi
    if [ ! -d data/${eval_set} ]; then
        cp -r ${feat_dir}/data/voxceleb1_test data/
    fi
    # 1. Divide data/voxceleb2Naug into data/voxceleb2Naug_{train,dev}
    if [ ! -f ./data/voxceleb2Naug/${train_set}.uttlist ] && [ ! -f ./data/voxceleb2Naug/${dev_set}.uttlist ] && [ ! -f ./data/voxceleb2Naug/utt2spklab ] ; then
        separate_trainNdev_uttlists.py voxceleb2Naug ${train_set} ${dev_set} 0.04
    fi
    ## Extract subsets for train_train and train_dev
    for subset in ${train_set} ${dev_set}; do
        if [ ! -d data/${subset} ]; then
            subset_data_dir.sh --utt-list data/voxceleb2Naug/${subset}.uttlist data/voxceleb2Naug data/${subset}
        fi
    done
fi
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    ...
    compute-cmvn-stats scp:data/${train_set}/feats.scp data/${train_set}/cmvn.ark # I can use whole voxceleb2Naug data for this but this won't make a big difference with this large amount of data
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    ...
    for subset in ${train_set} ${dev_set}; do
        # update json
        local/update_json_general.sh --loc output --k spklab ${dumpdir}/${subset}/data.json ./data/voxceleb2Naug/utt2spklab
    done
fi
...
echo The number of speakers for spkid training: ${num_spk} # should be 6114 for voxceleb2Naug
