...
# JJ: Edited from run_backendonly_cpuparallel.voxceleb2plda_voxceleb1eval.sh
...
#fi
train_set=voxceleb2Naug_train
dev_set=voxceleb2Naug_dev
...
if [ ${stage} -le 6 ] && [ ${stop_stage} -ge 6 ]; then
  ...
  if [ ! -d dump/voxceleb2Naug_trainNdev ]; then
    mkdir -p dump/voxceleb2Naug_trainNdev
  fi
  cat dump/voxceleb2Naug_{train,dev}/emb.${embname} > dump/voxceleb2Naug_trainNdev/emb.${embname}
  rm dump/voxceleb2Naug_{train,dev}/emb.${embname}

  # to combine utt2spk and spk2utt
  if [ ! -d data/voxceleb2Naug_trainNdev ]; then
    combine_data.sh data/voxceleb2Naug_trainNdev data/voxceleb2Naug_train data/voxceleb2Naug_dev
  fi


  $train_cmd dump/voxceleb2Naug_trainNdev/log/compute_emb_mean_${embname}.log \
      ivector-mean ark:dump/voxceleb2Naug_trainNdev/emb.${embname} \
      dump/voxceleb2Naug_trainNdev/emb_mean.vec.${embname} || exit 1;

  # This script uses LDA to decrease the dimensionality prior to PLDA.
  lda_dim=150
  $train_cmd dump/voxceleb2Naug_trainNdev/log/lda.${embname}.log \
    ivector-compute-lda --total-covariance-factor=0.0 --dim=$lda_dim \
    "ark:ivector-subtract-global-mean ark:dump/voxceleb2Naug_trainNdev/emb.${embname} ark:- |" \
    ark:data/voxceleb2Naug_trainNdev/utt2spk dump/voxceleb2Naug_trainNdev/emb_transform.mat.${embname} || exit 1;

  # Train an out-of-domain PLDA model.
  $train_cmd dump/voxceleb2Naug_trainNdev/log/plda.${embname}.log \
    ivector-compute-plda ark:data/voxceleb2Naug_trainNdev/spk2utt \
    "ark:ivector-subtract-global-mean ark:dump/voxceleb2Naug_trainNdev/emb.${embname} ark:- | transform-vec dump/voxceleb2Naug_trainNdev/emb_transform.mat.${embname} ark:- ark:- | ivector-normalize-length ark:-  ark:- |" \
    dump/voxceleb2Naug_trainNdev/plda.${embname} || exit 1;
fi
# stage 9
if [ ${stage} -le 7 ] && [ ${stop_stage} -ge 7 ]; then
  ...
  $train_cmd exp/scores/log/spkid_scoring_${embname}.log \
    ...
    "ivector-copy-plda --smoothing=0.0 dump/voxceleb2Naug_trainNdev/plda.${embname} - |" \
    "ark:ivector-subtract-global-mean dump/voxceleb2Naug_trainNdev/emb_mean.vec.${embname} ark:dump/${eval_set}/emb.${embname} ark:- | transform-vec dump/voxceleb2Naug_trainNdev/emb_transform.mat.${embname} ark:- ark:- | ivector-normalize-length ark:- ark:- |" \
    "ark:ivector-subtract-global-mean dump/voxceleb2Naug_trainNdev/emb_mean.vec.${embname} ark:dump/${eval_set}/emb.${embname} ark:- | transform-vec dump/voxceleb2Naug_trainNdev/emb_transform.mat.${embname} ark:- ark:- | ivector-normalize-length ark:- ark:- |" \
