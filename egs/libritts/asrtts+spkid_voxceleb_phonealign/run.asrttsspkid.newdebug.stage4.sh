...
# JJ: Edited from run.asrttsspkid.sh
...
# config files
train_config=conf/train_pytorch_forwardtacotron2+spkemb_2flstm512Nusecat_nceloss_fullyunsync_shufflebatching_bs56.yaml
...
# Speaker id related
spkloss_weight=0.03
nceloss_weight=0.001 # the best value found in voxceleb1 only training
...
train_set=voxceleb_train_train
dev_set=voxceleb_train_dev
...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
    ...
    ## Get ori text file (voxceleb1_all: voxceleb1_{train_filtered,test}, voxcelelb2_all: voxceleb2_{train,test})
    ...
    ## voxceleb1_test is also included in the data/voxceleb_train/text above so remove them
    awk '{print $1}' data/${eval_set}/wav.scp > data/${eval_set}/uttlist
    ...
    echo "After removing utterances in data/${eval_set}/uttlist, the number of lines in each file at data/voxceleb_train/"
    for fname in `ls -p data/voxceleb_train/ | grep -v /`;do
        wc -l data/voxceleb_train/${fname}
    done
    utils/fix_data_dir.sh data/voxceleb_train

    # 2. Divide data/voxceleb_train into data/voxceleb_train_{train,dev}
    if [ ! -f ./data/voxceleb_train/${train_set}.uttlist ] && [ ! -f ./data/voxceleb_train/${dev_set}.uttlist ] && [ ! -f ./data/voxceleb_train/utt2spklab ] ; then
        separate_trainNdev_uttlists.py voxceleb_train ${train_set} ${dev_set} 0.04
    fi
    ## Extract subsets for train_train and train_dev
    for subset in ${train_set} ${dev_set}; do
        if [ ! -d data/${subset} ]; then
            subset_data_dir.sh --utt-list data/voxceleb_train/${subset}.uttlist data/voxceleb_train data/${subset}
        fi
    done
fi
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    ...
    compute-cmvn-stats scp:data/${train_set}/feats.scp data/${train_set}/cmvn.ark # I can use whole voxceleb_train data for this but this won't make a big difference with this large amount of data
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    ...
    for subset in ${train_set} ${dev_set}; do
        # update json
        local/update_json_general.sh --loc output --k spklab ${dumpdir}/${subset}/data.json ./data/voxceleb_train/utt2spklab
    done
fi
...
echo The number of speakers for spkid training: ${num_spk} # should be 7325 for voxceleb_train (voxceleb1 and 2) without voxceleb1_test
...
expname=${expname}_spkloss_weight${spkloss_weight}_nceloss_weight${nceloss_weight}_fullyunsync_debug
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
    ...
    CUDA_VISIBLE_DEVICES=${gpu_ix} python -m ipdb /export/b18/jcho/espnet3/espnet/bin/tts_train_speakerid.py \
           --num-spk ${num_spk} \
