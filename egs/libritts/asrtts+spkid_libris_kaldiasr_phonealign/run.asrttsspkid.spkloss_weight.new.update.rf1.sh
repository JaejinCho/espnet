...
# /export/b18/jcho/espnet3/egs/libritts/asrtts+spkid/run.asrttsspkid.spkloss_weight.new.update.sh
# w/o more code modification, reduction factor 1 makes more sense (actuallry
# correct) in kaldi phone alignment setting
...
# config files
train_config=conf/train_pytorch_tacotron2+spkemb_noatt_rf1.yaml
...
train_set_ori=../tts_featext/data/train_clean_460
...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
    ...
    ### JJ add - start (TODO: Code to replace text)
    for subset in ${train_set} ${dev_set};do
        awk '{$1=$1;print}' /export/b11/jcho/kaldi_20190316/egs/librispeech/s5/exp/chain_cleaned/tdnn_1d_sp/decode_train_clean_460_frame_subsampling_factor_3/phone.perframe.expand.sym.txt | rev | cut -d' ' -f3- | rev > data/${subset}/text # JJ: remove last 2 phonemes in each utterance to match # frames with # frames in fbank features
        # remove the lines where text is empty and fix the directory
        mv data/${subset}/text data/${subset}/.text && awk '{if(!(NF==1))print $0}' data/${subset}/.text > data/${subset}/text # JJ: This one do nothing in the kaldiali case
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    ...
    for subset in ${train_set} ${dev_set}; do
        # update json
        local/update_json_general.sh --loc output --k spklab ${dumpdir}/${subset}/data.json data/train_clean_460/train_trainNdev.utt2spklab
    done
fi
