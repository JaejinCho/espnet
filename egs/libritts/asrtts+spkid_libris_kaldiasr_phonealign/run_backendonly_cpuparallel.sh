...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
    ...
    (
        ...
        ${train_cmd} JOB=1:${nj} log/speakerid_decode_${embname}_${extract_set}.JOB.log \
        speakerid_decode.py \
            --backend ${backend} \
            --ngpu ${ngpu} \
            --verbose ${verbose} \
...
if [ ${stage} -le 8 ] && [ ${stop_stage} -ge 8 ]; then
  ...
  qsub -cwd -o log/qsub.run.eer_mindcf_${embname}.log -e log/qsub.run.eer_mindcf_${embname}.log -l mem_free=200G,ram_free=200G run.eer_mindcf.sh --embname ${embname}
fi
### JJ - end
