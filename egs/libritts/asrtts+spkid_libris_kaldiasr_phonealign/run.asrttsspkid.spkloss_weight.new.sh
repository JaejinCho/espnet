...
# /export/b18/jcho/espnet3/egs/libritts/asrtts+spkid/run.asrttsspkid.spkloss_weight.new.sh
...
# config files
train_config=conf/train_pytorch_tacotron2+spkemb_noatt.yaml
...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
    ...
    rm -f data/train_clean_460/{train,cv}.spkid.uttlist # they are not for libritts experiment (maybe delete later them in the original directory)
    ...
    for subset in ${train_set} ${dev_set};do
        cp /export/b11/jcho/kaldi_20190316/egs/librispeech/s5/exp/chain_cleaned/tdnn_1d_sp/decode_train_clean_460_frame_subsampling_factor_3/phone.perframe.expand.sym.txt data/${subset}/text
