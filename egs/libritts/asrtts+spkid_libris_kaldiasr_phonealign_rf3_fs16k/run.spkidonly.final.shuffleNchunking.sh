...
# JJ: edited from run.spkidonly.final.sh. (+) add random batching and chunking
# JJ: edited from run.spkidonly.8.sh  (+) Parameters are changed for same
# optimizer used for tts+spkid training (i.e. to use simple Adam with its default setting: amsgrad removed,
# weight-decay removed,ALL ARGS LISTED HERE ARE EITHER DETERMINED BY CONFIG
# FILE OR AS DEFAULT VALUES)
# JJ: edited from run.spkidonly.7.sh  (+) Report 'lr' in log files
# JJ: edited from run.spkidonly.6.sh  (+) Nanxin's optim-like scheduling for lr
# JJ: edited from run.spkidonly.5.sh. (+) batch_sort_key and chunking
# JJ: edited from run.spkidonly.4.sh. amsgrad and epochs are added to expname
# JJ: edited from run.spkidonly.3.sh. eps and wd arguments are added (do diff
# for cmpr)
# JJ: edited from run.spkidonly.2.sh. lr argument is added (do diff for cmpr)
# JJ: edited from run.spkidonly.sh (do diff for cmpr). classification acc included
...
expname=${expname}_speakeridonly_ttslossw${ttsloss_weight}_spklossw${spkloss_weight}_batchsort${batch_sort_key}_chunking${chunking}_final
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
    ...
    ${cuda_cmd} --gpu ${ngpu} ${expdir}/train.log \
        tts_train_speakeridonly_temp.py \
           --model-module espnet.nets.pytorch_backend.e2e_tts_tacotron2_speakeridonly_temp:Tacotron2 \
           --num-spk ${num_spk} \
