...
# JJ: Copied from run.asrttsspkid.spkloss_weight.new.update.rf3.sh
# JJ: Copied and edited from run.asrttsspkid.spkloss_weight.new.update.rf1.sh
# predicted output seq will be longer than the original acoustic seq (by 2 to 4
# frames) >> No need to edit anything from espnet codes (*update*). Only
# modification in this script (regarding text) and train_config is enough
...
# feature extraction related (same as voxceleb's.)
...
train_set_ori=../tts_featext/data/train_clean_460_16k
train_set=train_train_16k
dev_set=train_dev_16k
eval_set=train_eval_16k
...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
    ...
    for fpath in `ls data/train_clean_460_16k/*uttlist`;do
        dname=$(basename ${fpath} | cut -d'.' -f1)_16k # this is same as dname=$(basename ${fpath%.*})
        subset_data_dir.sh --utt-list ${fpath} data/train_clean_460_16k/ data/${dname}/
        ls -A data/${dname}/*
    done
    for subset in ${train_set} ${dev_set};do
        cp /export/b18/jcho/espnet3/egs/libritts/asrtts+spkid_libris_kaldiasr_phonealign_rf3/data/${subset%_16k}/text data/${subset}/text # # phone ali seq * 3 - (2, 3 or 4) = # acoustic seq
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    ...
    sed -e '1 s:<eps> 0:<unk> 0:' ${dict} | awk -F' ' '{$2=$2+1;print}' OFS=' ' > .tmpdict && mv .tmpdict ${dict} # in sed, 1 means only apply it to the first line
    # JJ add - end
    wc -l ${dict}
    ...
    for subset in ${train_set} ${dev_set}; do
        # update json
        local/update_json_general.sh --loc output --k spklab ${dumpdir}/${subset}/data.json data/train_clean_460_16k/train_trainNdev.utt2spklab
    done
fi
