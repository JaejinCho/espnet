...
if [[ model.loss.best == ${model} ]]; then
    ...
    embname=`basename ${expdir}`_model.${model}
fi
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
    ...
    (
        ...
        outemb=dump/${extract_set}/emb_${embname} # changed "." to "_" in the name since spkloss_weight could include decimal point
        if [ ! -d log/speakerid_decode ]; then
            mkdir -p log/speakerid_decode
            echo "This directory is to keep log files for speaker embedding extraction" > log/speakerid_decode/Readme.txt
        fi
        ${train_cmd} JOB=1:${nj} log/speakerid_decode/speakerid_decode_${embname}_${extract_set}.splitixJOB.log \
        speakerid_decode.py \
            --backend ${backend} \
            --ngpu ${ngpu} \
            --verbose ${verbose} \
            ...
            --out-file ${outemb}.splitixJOB

        cat ${outemb}.splitix* | sort -k1 > ${outemb} # changed "." to "_" in the name since spkloss_weight could include decimal point
        rm ${outemb}.splitix*
    ) &
...
if [ ${stage} -le 6 ] && [ ${stop_stage} -ge 6 ]; then
  ...
  cat dump/train_{train,dev}/emb_${embname} > dump/train_trainNdev/emb_${embname}
  ...
  $train_cmd dump/train_trainNdev/log/compute_emb_mean_${embname}.log \
      ivector-mean ark:dump/train_trainNdev/emb_${embname} \
      dump/train_trainNdev/emb_mean.vec_${embname} || exit 1;
  ...
  # transformation)
  lda_dim=150
  $train_cmd dump/train_trainNdev/log/lda_${embname}.log \
    ivector-compute-lda --total-covariance-factor=0.0 --dim=$lda_dim \
    "ark:ivector-subtract-global-mean ark:dump/train_trainNdev/emb_${embname} ark:- |" \
    ark:data/train_trainNdev/utt2spk_actual dump/train_trainNdev/emb_transform.mat_${embname} || exit 1;
  ...
  $train_cmd dump/train_trainNdev/log/plda_${embname}.log \
    ivector-compute-plda ark:data/train_trainNdev/spk2utt_actual \
    "ark:ivector-subtract-global-mean ark:dump/train_trainNdev/emb_${embname} ark:- | transform-vec dump/train_trainNdev/emb_transform.mat_${embname} ark:- ark:- | ivector-normalize-length ark:-  ark:- |" \
    dump/train_trainNdev/plda_${embname} || exit 1;
fi
# stage 9
if [ ${stage} -le 7 ] && [ ${stop_stage} -ge 7 ]; then
  ...
  $train_cmd exp/scores/log/spkid_scoring_${embname}.log \
    ...
    "ivector-copy-plda --smoothing=0.0 dump/train_trainNdev/plda_${embname} - |" \
    "ark:ivector-subtract-global-mean dump/train_trainNdev/emb_mean.vec_${embname} ark:dump/${eval_set}/emb_${embname} ark:- | transform-vec dump/train_trainNdev/emb_transform.mat_${embname} ark:- ark:- | ivector-normalize-length ark:- ark:- |" \
    "ark:ivector-subtract-global-mean dump/train_trainNdev/emb_mean.vec_${embname} ark:dump/${eval_set}/emb_${embname} ark:- | transform-vec dump/train_trainNdev/emb_transform.mat_${embname} ark:- ark:- | ivector-normalize-length ark:- ark:- |" \
...
if [ ${stage} -le 8 ] && [ ${stop_stage} -ge 8 ]; then
  ...
  qsub -cwd -o log/qsub/qsub.run.eer_mindcf_${embname}.log -e log/qsub/qsub.run.eer_mindcf_${embname}.log -l mem_free=200G,ram_free=200G run.eer_mindcf.sh --embname ${embname}
fi
### JJ - end
