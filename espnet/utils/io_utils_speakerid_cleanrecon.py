# Edited from espnet/utils/io_utils_speakerid.py
# (TODO) Check if parts added/edited from the above are working as expected in the whole pipeline
...
class LoadInputsAndTargets(object):
        ...
        for uttid, info in batch:
            ...
            if self.load_input:
                ...
                for idx, inp in enumerate(info['input']):
                    ...
                    if arklink == 'None': # possible option only for inp['featori']
                        x_feats_dict.setdefault(inp['name'], []).append(None)
                    else:
                        x = self._get_from_loader(
                            filepath=arklink,
                            filetype=inp.get('filetype', 'mat'))
                        x_feats_dict.setdefault(inp['name'], []).append(x)
            # FIXME(kamo): Dirty way to load only speaker_embedding without the other inputs # JJ (?): When is it used?
    ...
    def _create_batch_tts(self, x_feats_dict, y_feats_dict, uttid_list, eos):
        ...
        if self.load_input:
            if len(x_feats_dict) == 1:
                ys = list(x_feats_dict.values())[0]
                assert len(xs) == len(ys), (len(xs), len(ys))
                ys = [ys[i] for i in nonzero_sorted_idx]
            else:
                assert len(x_feats_dict) == 2, len(x_feats_dict) # currently only support this
                ys, ys_b4noise = list(x_feats_dict.values())
                assert len(xs) == len(ys), (len(xs), len(ys))
                ys = [ys[i] for i in nonzero_sorted_idx]
                assert len(xs) == len(ys_b4noise), (len(xs), len(ys_b4noise))
                ys_b4noise = [ys_b4noise[i] for i in nonzero_sorted_idx]
            ...
            if len(x_feats_dict) == 1:
                ...
                y_name, y_b4noise_name = list(x_feats_dict.keys())
                return_batch = OrderedDict([(x_name, xs),
                                            (y_name, ys),
                                            (y_b4noise_name, ys_b4noise),
                                            (spembs_name, spembs),
                                            (spklabs_name, spklabs),
                                            (spcs_name, spcs)])

        elif self.use_speaker_embedding: # JJ: when only speaker embedding is input (TODO: maybe need to change later)
