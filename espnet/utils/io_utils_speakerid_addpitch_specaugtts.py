# JJ: Edited from espnet/utils/io_utils_speakerid_addpitch.py
...
class LoadInputsAndTargets(object):
        ...
        if self.preprocessing is not None:
            ...
            for x_name in return_batch.keys():
                if x_name.startswith("input"):
                    temp_names.append(x_name)
            for x_name in temp_names:
                return_batch[x_name + '_aug'] = self.preprocessing(
                        deepcopy(return_batch[x_name]), uttid_list, **self.preprocess_args)
    ...
    def _create_batch_tts(self, x_feats_dict, y_feats_dict, uttid_list, eos):
        ...
        if self.load_input:
            ...
            if self.use_speaker_embedding: # (JJ): More conditions will be added if needed
                if (not self.train_spkid_extractor):
                    spembs = list(x_feats_dict.values())[1]
                    spembs = [spembs[i] for i in nonzero_sorted_idx]
                    spembs_name = list(x_feats_dict.keys())[1]
                elif self.add_pitch:
                    pitchs = list(x_feats_dict.values())[1]
                    assert len(xs) == len(pitchs), (len(xs), len(pitchs))
                    pitchs = [pitchs[i] for i in nonzero_sorted_idx]
                #else: # (JJ) will be added if needed.
