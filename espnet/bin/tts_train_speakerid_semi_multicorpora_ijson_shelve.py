...
# Edited from ../../../espnet/bin/tts_train_speakerid_semi_multicorpora.py
# Edited from ../../../espnet/bin/tts_train_speakerid_semi.py
# Edited from espnet/bin/tts_train_speakerid.py
...
def get_parser():
    ...
    parser.add_argument('--trainer_update', default=True, type=strtobool,
                        help='Whether to update trainer when resuming from a snapshot')
    ...
    # task related
    parser.add_argument('--train-json-nolab', default=None, type=str,
                        help='Filename of training json where there are NO spk labels')
    ...
    ## JJ: commented out the below add_argument for now
    #parser.add_argument('--ratio-nolab2lab', default=1, type=int,
    #                    help='ratio of # nolab examples to # lab examples in training batchset after upsampling lab examples')
    ...
    parser.add_argument("--train-spk-embed-dim", default=None, type=int,
                        help="Number of speaker embedding dimensions in training speaker id module")
    parser.add_argument('--num-spk', default=0, type=int,
                        help='The number of speakers in speaker embedding training')
    parser.add_argument('--spkloss-weight', default=0, type=float,
                        help='Scaling hyperparmeter for loss between TTS and speaker id')
    # speaker uniform sampling in building batches
    parser.add_argument('--sample-uniform-spk', default=False, type=strtobool,
                        help='Whether to sample speakers uniformly in training')
    parser.add_argument('--total-num-utt-inbatches', default=0, type=int,
                        help='final # utts we want to have after the uniform speaker sampling')
    # JJ added - end
    ...
    parser.add_argument('--opt', default='adam', type=str,
                        choices=['adam', 'noam'],
                        help='Optimizer')
    parser.add_argument('--pytorch-scheduler-type', default=None, type=str,
                        choices=['ReduceLROnPlateau'],
                        help='Pytorch scheduler type')
    parser.add_argument('--lr-reduce-factor', default=0.33, type=float,
                        help='Reduce facotr when decreasing lr by scheduler')
    parser.add_argument('--lr-reduce-patience', default=1, type=int,
                        help='Number of epochs to wait without improvement before reducing the lr by scheduler')
    ...
    # data structure related for training
    parser.add_argument('--shelve-train-batchset', default=None, type=str,
                        help='shelve path that stores the output of make_batchset for training set')
    parser.add_argument('--shelve-valid-batchset', default=None, type=str,
                        help='shelve path that stores the output of make_batchset for validation set')
    parser.add_argument('--save-shelve-dir', default=None, type=str,
                        help='shelve directory to save batchsets. Shuold be distinguiable between different batchset generations')
...
def main(cmd_args):
    ...
    if args.backend == "pytorch":
        from espnet.tts.pytorch_backend.tts_speakerid_semi_multicorpora_ijson_shelve import train
        train(args)
    else:
