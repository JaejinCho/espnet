...
# edited from espnet/bin/tts_decode.py
...
def get_parser():
    ...
    parser.add_argument('--batch-size', '--batch-seqs', '-b', default=256, type=int,
                        help='Maximum seqs in a minibatch (0 to disable)')
    # task related
    parser.add_argument('--json', type=str, required=True,
                        help='Filename of json to evaluate a model on')
    parser.add_argument('--model', type=str, required=True,
                        help='Model file parameters to read')
    parser.add_argument('--model-conf', type=str, default=None,
                        help='Model config file')
    # decoding related (these are just place holder to run this script)
...
def main(args):
    ...
    if args.backend == "pytorch":
        #from espnet.tts.pytorch_backend.tts_speakerid import decode
        #decode(args)
        from utils.speakerid import eval_spkidclassification
        eval_spkidclassification(args)
    else:
