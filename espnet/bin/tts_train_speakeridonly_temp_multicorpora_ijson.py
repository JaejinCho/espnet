...
# JJ: /export/b18/jcho/espnet3/espnet/bin/tts_train_speakeridonly_temp.py to include multicorpora json loading with ijson
...
def get_parser():
    ...
    parser.add_argument('--model-module', type=str, default="espnet.nets.pytorch_backend.e2e_tts_tacotron2_speakeridonly_temp:Tacotron2",
    ...
    # JJ added - start
    parser.add_argument('--spkid-warmup', default=-1, type=int, nargs='?',
                        help="whether to enable Nanxin's spkid-like lr scheduling. -1 means disabled")
    parser.add_argument('--chunking', default=False, type=strtobool,
                        help='Chunking in bathcing. This is for speaker ID')
    ...
    parser.add_argument('--spkloss-weight', default=1, type=float,
                        help='Scaling hyperparmeter for loss between TTS and speaker id')
    parser.add_argument('--ttsloss-weight', default=0, type=float,
                        help='Scaling hyperparmeter for tts loss between TTS and speaker id')
    # moco
    parser.add_argument('--moco', default=False, type=strtobool,
                        help='Whether to use Momentum Contrastive in training')
    parser.add_argument('--K', default=38912, type=int,
                        help='The size of queue to accumulate spkembs. Should be [int] * [batchsize]. The default value is 512 * 76')
    parser.add_argument('--m', default=0.999, type=float,
                        help='momentum weight for momentum encoder in MoCo')
    parser.add_argument('--T', default=0.1, type=float,
                        help='temperature for contrastive softmax')
    ...
    parser.add_argument('--amsgrad', default=False, type=strtobool,
                        help='Whether to use the function in Adam optimizer (follow Nanxins optim setting)')
...
def main(cmd_args):
    ...
    if args.backend == "pytorch":
        from espnet.tts.pytorch_backend.tts_speakeridonly_temp_multicorpora_ijson import train
        train(args)
    else:
