...
# JJ: edited from espnet/nets/pytorch_backend/tacotron2/decoder_update.py
# JJ: Decoder_noatt added from espnet/nets/pytorch_backend/tacotron2/decoder.py
...
class Decoder_forward(torch.nn.Module):
    """Decoder_forward module of Spectrogram prediction network.
    (JJ) Edited from Decoder_noatt to implement a decoder of the forwardtacotron2
    (JJ) This is a modified version from Decoder class without attention related parts
    when your TTS inputs are phoneme alignments for example
    """
    ...
    def __init__(self, idim, odim,
        ...
        Args:
            ...
            zoneout_rate (float, optional): Zoneout rate.
            reduction_factor (int, optional): Reduction factor.
            frame_subsampling_factor (int, optional): Frame subsampling factor (in Kaldi ASR)
            rnntype (str, optional): RNN type (between lstm and gru)
			bidirection (bool, optional): Whether to use bidirectional lstm
        ...
        # define lstm network
        prenet_units = prenet_units if prenet_layers != 0 else odim
        # (JJ): From https://github.com/as-ideas/ForwardTacotron, args.dunits = 512
        if rnntype == 'lstm':
            self.lstm_fwd = torch.nn.LSTM(idim + prenet_units, dunits, dlayers, batch_first=True, bidirectional=True if bidirection else False)
        elif rnntype == 'gru':
            self.lstm_fwd = torch.nn.GRU(idim + prenet_units, dunits, dlayers, batch_first=True, bidirectional=True if bidirection else False)
        else:
            raise ValueError('rnntype should be either lstm or gru')
        ...
        # define projection layers
        if self.use_concate:
            iunits = 2*dunits + idim if bidirection else idim + dunits
        else: # for this, I can try either using lstm output only or using lstm ouput + encoder embedding
            iunits = 2*dunits if bidirection else dunits
        self.feat_out = torch.nn.Linear(iunits, odim * reduction_factor, bias=False) # (JJ): For now, only reduction factor 1 is possible (subsample is compensated by length regulator before this)
        self.prob_out = torch.nn.Linear(iunits, reduction_factor)
    ...
    def forward(self, hs, hlens, ys):
        ...
        """
		### JJ add - start
        # (no need) initialize hidden states of decoder (pytorch default is to initialize z_0, c_0 to zero)
		# run the decoder
        ys = ys.roll(1,1) # shift to the right once
        ys[:,0,:] = torch.zeros(ys[:,0,:].shape) # first time-steps are zerod
        prenet_out = self.prenet(ys) # teacher forcing. (TODO hp (JJ): I guess this should shifted to right once)
        # upsample (what length regulator does in fastspeech)
        len_cut = prenet_out.size(1)
        hs = [ h.repeat(1,self.frame_subsampling_factor).view(self.frame_subsampling_factor*h.size(0),h.size(1))[:len_cut] for h in hs ] # (JJ) I guess hs are zero-padded already
        hs = torch.stack(hs,dim=0)
        xs = torch.cat([hs, prenet_out], dim=2) # (TODO hp (JJ): Maybe concating prenet_out is making the problem too easy)
        zcs, _ = self.lstm_fwd(xs)
        if self.use_concate:
            zcs = torch.cat([zcs, hs], dim=2) # (TODO hp (JJ): I can try cat or sum here (If I do cat this will be the same as tacotron2_noatt (EXCEPT lstm is blstm, dropout and upsampling in a difference place))
        zcs = F.dropout(zcs, p=0.1, training=self.training) # dropout rate is followed by: https://github.com/as-ideas/ForwardTacotron
        before_outs = self.feat_out(zcs) # (B, Lmax, odim)
        if self.postnet is not None:
            after_outs = before_outs.transpose(1,2) + self.postnet(before_outs.transpose(1,2))  # (B, odim, Lmax)
            after_outs = after_outs.transpose(1,2)
        else:
            after_outs = before_outs

        logits = self.prob_out(zcs).squeeze(2) # (B, Lmax). (TODO) JJ: This might need to change when reduction_factor != 1
		### JJ add - end
        ...
        return after_outs, before_outs, logits, None # None for att_ws
