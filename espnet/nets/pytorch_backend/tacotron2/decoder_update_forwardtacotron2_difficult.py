...
# JJ: edited from espnet/nets/pytorch_backend/tacotron2/decoder_update_forwardtacotron2.py
...
class Decoder_noatt(torch.nn.Module):
    """Decoder_noatt module of Spectrogram prediction network.
    ...
    def __init__(self, idim, odim,
        ...
        """
        super(Decoder_noatt, self).__init__()
    ...
    def forward(self, hs, hlens, ys):
        ...
        for ix, y in enumerate(ys.transpose(0, 1)):
            ...
            for l in six.moves.range(1, len(self.lstm)):
                z_list[l], c_list[l] = self.lstm[l](
                    z_list[l - 1], (z_list[l], c_list[l]))
            zcs = torch.cat([z_list[-1], att_c], dim=1) if self.use_concate else z_list[-1]
            outs += [self.feat_out(zcs).view(hs.size(0), self.odim, -1)] # JJ: in view, Does interchanging self.odim and -1 and change the dimension back again later matter? (I do not think so)
            logits += [self.prob_out(zcs)]
            prev_out = y  # teacher forcing
        ...
        if self.reduction_factor > 1: # JJ: What is this? why do we need this?
            before_outs = before_outs.view(before_outs.size(0), self.odim, -1)  # (B, odim, Lmax)
...
class Decoder_forward(torch.nn.Module):
    ...
    def __init__(self, idim, odim,
        ...
        Args:
            ...
            dlayers (int, optional): The number of decoder lstm layers.
            dunits (int, optional): The number of decoder lstm units.
            (NOT used) prenet_layers (int, optional): The number of prenet layers.
            (NOT used) prenet_units (int, optional): The number of prenet units.
            postnet_layers (int, optional): The number of postnet layers.
            postnet_filts (int, optional): The number of postnet filter size.
            postnet_chans (int, optional): The number of postnet filter channels.
            output_activation_fn (torch.nn.Module, optional): Activation function for outputs.
        ...
        if rnntype == 'lstm':
            self.lstm_fwd = torch.nn.LSTM(idim, dunits, batch_first=True, bidirectional=bidirection)
        elif rnntype == 'gru':
            self.lstm_fwd = torch.nn.GRU(idim, dunits, batch_first=True, bidirectional=bidirection)
        else:
    ...
    def forward(self, hs, hlens, ys):
        ...
        hs, _ = self.lstm_fwd(hs)
        hs = F.dropout(hs, p=0.1, training=self.training) # dropout rate is followed by: https://github.com/as-ideas/ForwardTacotron
        before_outs = self.feat_out(hs) # (B, Lmax, odim)
        if self.postnet is not None:
        ...
        logits = self.prob_out(hs).squeeze(2) # (B, Lmax). (TODO) JJ: This might need to change when reduction_factor != 1
		### JJ add - end
