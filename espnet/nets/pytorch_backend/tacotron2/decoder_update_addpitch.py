...
class Postnet(torch.nn.Module):
    ...
    def __init__(self, idim, odim, n_layers=5, n_chans=512, n_filts=5, dropout_rate=0.5, use_batch_norm=True):
        ...
        Args:
            idim (int): Dimension of the inputs. (JJ: This is not being used. No need)
            odim (int): Dimension of the outputs.
...
class Decoder_noatt(torch.nn.Module):
    ...
    def __init__(self, idim, odim,
        ...
        if prenet_layers > 0:
            self.prenet = Prenet(
                idim=odim if pitch_dim == 0 else odim + pitch_dim * reduction_factor,
                n_layers=prenet_layers,
                n_units=prenet_units,
                dropout_rate=dropout_rate)
        else:
    ...
    def forward(self, hs, hlens, ys, pitchs=None):
        """Calculate forward propagation.
        ...
        if type(pitchs) != type(None):
            assert pitchs.size(1) == ys.size(1), (pitchs.size(1), ys.size(1))
            pit_dim = pitchs.size(-1)

        if self.reduction_factor > 1:
            ys = ys[:, self.reduction_factor - 1::self.reduction_factor]
            if type(pitchs) != type(None):
                lento = self.reduction_factor * ys.size(1) # length we will extend to
                pitchs = pitchs[:, self.reduction_factor - 1:]
                lencur = pitchs.size(1) # current length
                lenpad = lento - lencur # length for padding
                if lenpad != 0:
                    assert lenpad > 0, "len. diff. is negative"
                    pad_vec = pitchs[:,-1,:].unsqueeze(1).expand(-1, lenpad, -1)
                    pitchs = torch.cat([pitchs,pad_vec],dim=1)
                    # (JJ) currently make a new feature by concat over context
                    # context window. Later I could try just use a first
                    # feature in the context window
                pitchs = pitchs.view(ys.size(0), -1, self.reduction_factor * pit_dim)
                assert ys.size(1) == pitchs.size(1), "seq. len. are different between ys: {0} and pitchs: {1} after process.".format(ys.size(1),pitchs.size(1))
        ...
        for _ in six.moves.range(1, len(self.lstm)):
            ...
            z_list += [self._zero_state(hs)]
        if type(pitchs) != type(None):
            prev_out = hs.new_zeros(hs.size(0), self.odim + self.reduction_factor * pit_dim)
        else:
            prev_out = hs.new_zeros(hs.size(0), self.odim)
        ...
        for ix, y in enumerate(ys.transpose(0, 1)):
            ...
            logits += [self.prob_out(zcs)]
            if type(pitchs) != type(None):
                prev_out = torch.cat([y, pitchs.transpose(0, 1)[ix]], dim=-1) # teacher forcing
            else:
                prev_out = y # teacher forcing
