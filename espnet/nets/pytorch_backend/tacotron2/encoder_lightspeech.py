...
# JJ: edited from nets/pytorch_backend/tacotron2/encoder.py
...
class Encoder_light(torch.nn.Module):
    ...
    def __init__(self, idim,
        ...
        Args:
            ...
            ernntype (str, optional) RNN type as either lstm or gru
            ebidirection (bool, optional) To set RNN bidirectional or not
        ...
        if elayers > 0:
            ...
            if ernntype == 'lstm':
                self.blstm = torch.nn.LSTM(
                    iunits, eunits, elayers,
                    batch_first=True,
                    bidirectional=True)
            elif ernntype == 'gru':
                self.blstm = torch.nn.GRU(
                    iunits, eunits, elayers,
                    batch_first=True,
                    bidirectional=True)
            else:
                raise ValueError('ernntype should be either lstm or gru')
        else:
            self.blstm = None

        # initialize. Q (JJ): Is this also initializing states in rnn? (I think we want them to be zeros for initial values (pytorch default))
