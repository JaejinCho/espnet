...
import logging
# Edited from speakerid.py to include normalization of spkembs in decoding
...
def SequenceGenerator(model, device, feat_scp, out_file):
    ...
    with torch.no_grad():
        for lab, x in read_mat_scp(feat_scp):
            y=[x]
            y_pred = F.normalize(model.predict(torch.from_numpy(np.array(y,dtype=np.float32)).to(device)),p=2,dim=1)
