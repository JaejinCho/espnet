...
scp=${data}/feats.fbankonly.scp

split_scps=""
for n in $(seq ${nj}); do
    split_scps="$split_scps $logdir/feats.fbankonly.$n.scp"
done
...
${cmd} JOB=1:${nj} ${logdir}/griffin_lim_${name}.JOB.log \
    convert_fbank_to_wav.py \
        ...
        --n_mels ${n_mels} \
        --iters ${iters} \
        scp:${logdir}/feats.fbankonly.JOB.scp \
        ${wavdir}

rm ${logdir}/feats.fbankonly.*.scp 2>/dev/null
