...
# Edited from speakerid.py

# shuffle BN related - start: reference at https://github.com/facebookresearch/SlowFast/blob/master/slowfast/models/batchnorm_helper.py#L37-L46
class SubBatchNorm1d(nn.Module):
    """
    The standard BN layer computes stats across all examples in a GPU. In some
    cases it is desirable to compute stats across only a subset of examples
    (e.g., in multigrid training https://arxiv.org/abs/1912.00998).
    SubBatchNorm1d splits the batch dimension into N splits, and run BN on
    ...
    def __init__(self, num_splits, **args):
        ...
        """
        super(SubBatchNorm1d, self).__init__()
        ...
        if args.get("affine", True):
            ...
            self.affine = False
        self.bn = nn.BatchNorm1d(**args)
        args["num_features"] = num_features * num_splits
        self.split_bn = nn.BatchNorm1d(**args)
    ...
    def forward(self, x): 
        if len(x.shape) == 3:
            if self.training:
                n, c, w = x.shape # (TODO (JJ): Check if the dimension ordered in a correct way)
                x = x.view(n // self.num_splits, c * self.num_splits, w)
                x = self.split_bn(x)
                x = x.view(n, c, w)
            else:
                x = self.bn(x)
            if self.affine: # (TODO (JJ): Check this part. I think this is just linear transformation)
                x = x * self.weight.view((-1, 1))
                x = x + self.bias.view((-1, 1))

        elif len(x.shape) == 2:
            if self.training:
                n, w = x.shape # (TODO (JJ): Check if the dimension ordered in a correct way)
                x = x.view(n // self.num_splits, w * self.num_splits)
                x = self.split_bn(x)
                x = x.view(n, w)
            else:
                x = self.bn(x)
            if self.affine: # (TODO (JJ): Check this part. I think this is just linear transformation)
                x = x * self.weight.view((-1))
                x = x + self.bias.view((-1))
        return x
...
class SubBatchNorm2d(nn.Module):
    ...
    def forward(self, x):
        if self.training:
            n, c, h, w = x.shape # (TODO (JJ): Check if the dimension ordered in a correct way)
...
def aggregate_sub_bn_stats(module):
    ...
    for child in module.children():
        if isinstance(child, SubBatchNorm1d) or isinstance(child, SubBatchNorm2d):
            child.aggregate_stats()
            count += 1
        else:
            count += aggregate_sub_bn_stats(child)
    return count
# shuffle BN related - end
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, BN_num_splits=4):
        super(BasicBlock, self).__init__()
        self.conv1 = conv3x3(inplanes, planes, stride)
        self.bn1 = SubBatchNorm2d(BN_num_splits, num_features=planes)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = conv3x3(planes, planes)
        self.bn2 = SubBatchNorm2d(BN_num_splits, num_features=planes)
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, **kwargs):
        ...
        self.conv1 = nn.Conv2d(1, 16, kernel_size=7, stride=1, padding=3,
                               bias=False)
        self.bn1 = SubBatchNorm2d(kwargs['BN_num_splits'], num_features=16)
        logging.warning("The number of splits for shuffle BN is {}".format(kwargs['BN_num_splits']))
        self.relu = nn.ReLU(inplace=True)
        #self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
        self.layer1 = self._make_layer(block, 16, layers[0], BN_num_splits=kwargs['BN_num_splits'])
        self.layer2 = self._make_layer(block, 32, layers[1], stride=2, BN_num_splits=kwargs['BN_num_splits'])
        self.layer3 = self._make_layer(block, 64, layers[2], stride=2, BN_num_splits=kwargs['BN_num_splits'])
        self.layer4 = self._make_layer(block, 128, layers[3], stride=2, BN_num_splits=kwargs['BN_num_splits'])
        ...
        for m in self.modules():
            ...
            # SubBatchNorm2d initializes its weights in generating the instance

    def _make_layer(self, block, planes, blocks, stride=1, BN_num_splits=4):
        downsample = None
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                nn.Conv2d(self.inplanes, planes * block.expansion,
                          kernel_size=1, stride=stride, bias=False),
                SubBatchNorm2d(BN_num_splits, num_features=planes * block.expansion),
            )

        layers = []
        layers.append(block(self.inplanes, planes, stride, downsample, BN_num_splits=BN_num_splits))
        self.inplanes = planes * block.expansion
        for i in range(1, blocks):
            layers.append(block(self.inplanes, planes, BN_num_splits=BN_num_splits))
...
class E2E_speakerid(nn.Module):
    def __init__(self, input_dim, output_dim, Q, D, hidden_dim=128, distance_type='norm', network_type='att', pooling='mean', regularization=None, asoftmax=False, resnet_AvgPool2d_fre_ksize = 3, BN_num_splits=4):
        ...
        self.res  = resnet34(AvgPool2d_fre_ksize = resnet_AvgPool2d_fre_ksize, BN_num_splits=BN_num_splits)
        ...
        self.bn1  = SubBatchNorm1d(BN_num_splits, num_features=hidden_dim)
    ...
    def forward(self, x):
        ...
        if self.asoftmax:
            ...
            x_final = F.relu(x_emb)
            x_final = self.fc2(x_final)
            return x_emb, F.log_softmax(x_final, dim=-1)
...
class AngleLoss(nn.Module):
    def __init__(self, gamma=0):
        ...
        #self.it = 560600
        #self.it = 200300
        #self.it = 193100 # Temporal change for one resume exp.
        #self.it = 165600
        logging.warning("Starting value of self.it in AngleLoss: {}".format(self.it))
        self.LambdaMin = 5.0
    ...
    def forward(self, input, target):
        ...
        # JJ logging - start
        #if self.training:
        #    logging.warning("spkid train loss: {}".format(loss)) # JJ logging
        #else:
        #    logging.warning("spkid val loss: {}".format(loss)) # JJ logging
        #logging.warning("angle loss it: {}".format(self.it)) # JJ logging
        # JJ logging -end
