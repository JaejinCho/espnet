...
class E2E_speakerid(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.pool(x)
        x = self.fc1(x)
        x_emb = self.bn1(x) # (TODO, JJ: Study this)
        if self.asoftmax:
            ...
            k = (2*theta/3.14159265).floor() # 2 here corresponds to m = 2 in SPKID AAM paper (I would like to spend some time to prove how this is derived for the general case as in https://github.com/clcarwin/sphereface_pytorch/blob/master/net_sphere.py)
            n_one = k*0.0 - 1 # This just generating -1 matrices having its size as k.size()
            phi_theta = (n_one**k) * cos_m_theta - 2*k
            cos_theta = cos_theta * xlen.view(-1,1) # This is |x|cos(\theta)
            phi_theta = phi_theta * xlen.view(-1,1) # This is |x|cos(m\theta)
