...
def _read_compressed_mat(fd, format, row_major = False):
  ...
  if not row_major:
      ...
      data = np.transpose(np.reshape(np.frombuffer(fd.read(rows*cols), dtype='uint8', count=rows*cols), newshape=(rows,cols)))
