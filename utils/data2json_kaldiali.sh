...
# JJ: To deal with kaldi alignment inputs in text (copied and edited from data2json.sh)
...
kaldiali="" # This will be given when data/*/text is composed of kaldi phone alignment
...
elif [ -n "kaldiali" ]; then
    cp ${dir}/text ${tmpdir}/output/token.scp
elif [ -n "${nlsyms}" ]; then
...
## 2-1. sort by 1st columns (JJ added to avoid error in running merge_scp2json.py)
for o_type in token tokenid shape text;do
    sort -k1 ${tmpdir}/output/${o_type}.scp > ${tmpdir}/output/${o_type}.scp.sorted && mv ${tmpdir}/output/${o_type}.scp.sorted ${tmpdir}/output/${o_type}.scp
done
