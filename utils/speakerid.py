...
class ResNet(nn.Module):
    ...
    :param: int AvgPool2d_fre: This is the kernel size for AvgPool2d (for self.avgpool) towards frequency dimension.
                               Default is set to 10 assuming 80 dimensional fbank is given as input. While going through layers from 1 to 4 (self.layer1 to self.layer4), the frequency dim gets down-sampled by 8. So to make 80/8 = 10 numbers to one value, the freq dimension (kwargs['AvgPool2d_fre_ksize'])in nn.AvgPool2d should be 10 (as default). 
                               It was 3 origianlly from Nanxin's code since he used 23 dim mfcc/filter bank feature. (e.g. ceil(23/8) = 3). For more examples, with 64 dimensional fbank, it is okay to set the kernel size 10. When 64/8 = 8 that is smaller than the kernel size 10, this pooling layer simply works like the one with kernel size 8. However, if it is a 83 dimensional fbank, which in the end will have ceil(83/8) = 11 that is larger than the kernel size 10, the pooling layer will ignore the value in the last dimension (more precisely, values in the last 1 column and 1 row (in H, W) in doing the average. In this case, the result might not be accurate although you still get one averaged value
    '''
