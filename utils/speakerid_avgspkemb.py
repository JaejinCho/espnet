# JJ: edited from ../../../utils/speakerid.py
import torch
...
class AngleLoss(nn.Module):
    def __init__(self, gamma=0):
        ...
        self.gamma   = gamma
        self.it = 0 # JJ logging (remove the comment after debugging)
        #self.it = 102800
        #self.it = 330600
        #self.it = 379500
        #self.it = 961200
        #self.it = 268600
        #self.it=411900 # Temporal change for one resume exp.
        #self.it = 193100 # Temporal change for one resume exp.
        #self.it = 165600 # JJ logging
...
def SequenceGenerator(model, device, feat_scp, out_file):
    ...
    with torch.no_grad():
        for lab, x in read_mat_scp(feat_scp):
            ...
            else: # average chunk-wise speaker embeddings after removing the first and last 0.5 second parts
                y_pred = 0
                chunk_len = 300 # 3 sec
                stride = 50 # 0.5 sec
                x = x[stride:-stride] # remove 0.5 s at the front and back respectively
                num_chunks = int((utt_len - 100 - chunk_len)/float(stride)) + 1 # 100 because I removed 0.5 s at fron and back
                # run chunk-wise spkemb calculation
                for i in range(num_chunks):
                    y = [x[stride*i:stride*i+chunk_len]]
                    y_pred = y_pred + model.predict(torch.from_numpy(np.array(y,dtype=np.float32)).to(device)).cpu().data.numpy().flatten()
                y_pred = y_pred/num_chunks # average
